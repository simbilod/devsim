...
TriangleEdgePairFromEdgeModel<DoubleType>::TriangleEdgePairFromEdgeModel(
    const std::string &edgemodel, RegionPtr rp)
    : TriangleEdgeModel(edgemodel + "_node0_x", rp,
                        TriangleEdgeModel::DisplayType::SCALAR),
      edgeModelName(edgemodel) {
  RegisterCallback(edgemodel);

  model_names = {{{edgemodel + "_node0_x", edgemodel + "_node0_y"},
                  {edgemodel + "_node1_x", edgemodel + "_node1_y"}}};
  new TriangleEdgeSubModel<DoubleType>(model_names[0][1], rp,
                                       TriangleEdgeModel::DisplayType::SCALAR,
                                       this->GetSelfPtr());
  new TriangleEdgeSubModel<DoubleType>(model_names[1][0], rp,
                                       TriangleEdgeModel::DisplayType::SCALAR,
                                       this->GetSelfPtr());
  new TriangleEdgeSubModel<DoubleType>(model_names[1][1], rp,
                                       TriangleEdgeModel::DisplayType::SCALAR,
                                       this->GetSelfPtr());
}

template <typename DoubleType>
void TriangleEdgePairFromEdgeModel<DoubleType>::calcTriangleEdgeScalarValues()
    const {
  ...
  for (size_t i = 0; i < 2; ++i) {
    ...
    std::const_pointer_cast<TriangleEdgeModel, const TriangleEdgeModel>(fieldx)
        ->SetValues(evx[i]);

    auto fieldy = reg.GetTriangleEdgeModel(model_names[i][1]);
    dsAssert(fieldy.get(), "UNEXPECTED");
    std::const_pointer_cast<TriangleEdgeModel, const TriangleEdgeModel>(fieldy)
        ->SetValues(evy[i]);
  }
}

template <typename DoubleType>
void TriangleEdgePairFromEdgeModel<DoubleType>::Serialize(
...
CreateTriangleEdgePairFromEdgeModel(const std::string &edgemodel,
                                    RegionPtr rp) {
  const bool use_extended = rp->UseExtendedPrecisionModels();
  return create_triangle_edge_model<
      TriangleEdgePairFromEdgeModel<double>,
      TriangleEdgePairFromEdgeModel<extended_type>>(use_extended, edgemodel,
                                                    rp);
}
