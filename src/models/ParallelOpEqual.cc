...
template <typename U> FPECheck::FPEFlag_t OpEqualPacket<U>::getFPEFlag() const {
  return fpeFlag_;
}

template <typename U> size_t OpEqualPacket<U>::getNumberProcessed() const {
  return num_processed_;
}

template <typename U>
OpEqualPacket<U>::OpEqualPacket(U &opequaltask)
    : opEqualTask_(opequaltask), fpeFlag_(FPECheck::getClearedFlag()),
      num_processed_(0) {}

// http://software.intel.com/en-us/forums/intel-threading-building-blocks/topic/70959/
template <typename U> void OpEqualPacket<U>::join(const OpEqualPacket<U> &omp) {
...
template <typename U> void OpEqualRun(U &task, size_t vlen) {
  ...
  if ((num_threads > 1) && vlen > task_size) {
    ...
    ///// TODO: This code should be generalized and made available for all
    ///thread scheduling
    ...
    while (b < e) {
      auto packet =
          std::shared_ptr<OpEqualPacket<U>>(new OpEqualPacket<U>(task));
      packets.push_back(packet);
      futures.push_back(std::async(
          std::launch::async, OpEqualRange<OpEqualPacket<U>>(*packet, b, e)));
...
template <typename U> void OpEqualRange<U>::operator()() {
