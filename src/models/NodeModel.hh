...
public:
  ...
  NodeModel(const std::string &, const RegionPtr, NodeModel::DisplayType,
  ...
  template <typename DoubleType> void SetNodeValue(size_t, DoubleType);
  ...
  NodeModel::DisplayType GetDisplayType() const { return displayType; }

  const char *GetDisplayTypeString() const {
    return DisplayTypeString[static_cast<size_t>(displayType)];
  }

  void SetDisplayType(NodeModel::DisplayType dt) { displayType = dt; }

  ConstNodeModelPtr GetConstSelfPtr() const { return myself.lock(); }

  NodeModelPtr GetSelfPtr() { return myself.lock(); }
