...
template <typename DoubleType> struct times_equal {
  void operator()(DoubleType &x, const DoubleType &y) const { x *= y; }
};

template <typename DoubleType> struct plus_equal {
  void operator()(DoubleType &x, const DoubleType &y) const { x += y; }
};

template <typename DoubleType> struct minus_equal {
  void operator()(DoubleType &x, const DoubleType &y) const { x -= y; }
};
...
template <typename T, typename DoubleType> class ScalarData {
public:
  typedef T reftype;
