...
TriangleEdgeModel::TriangleEdgeModel(const std::string &nm, const RegionPtr rp,
    ...
    : name(nm), myregion(rp), uptodate(false), inprocess(false),
      displayType(dt), model_data(rp->GetNumberTriangles() * 3) {
...
const TriangleEdgeScalarList<DoubleType> &
TriangleEdgeModel::GetScalarValues() const {
  CalculateValues();
...
void TriangleEdgeModel::SetValues(
    const TriangleEdgeScalarList<DoubleType> &nv) {
      ...
      for (size_t i = 0; i < number_nodes; ++i) {
        ...
        if (c != 0) {
          values[i] /= c;
             ...
             TriangleEdgeModel::InterpolationType::COUPLE) {
