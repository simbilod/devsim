...
public:
  explicit ModelDataHolder(size_t l)
      : double_uniform_value(0.0), length(l), type(MDtype::DOUBLE),
        is_uniform(true) {
  ...
  template <typename DoubleType> MDtype GetMDtype() const;

  template <typename DoubleType>
  const std::vector<DoubleType> &GetValues() const;

  template <typename DoubleType> const DoubleType &GetUniformValue();

  template <typename DoubleType> void SetUniformValue(const DoubleType &);

  template <typename DoubleType> void SetValue(size_t, const DoubleType &);

  template <typename DoubleType> std::vector<DoubleType> &Values();
  ...
  template <typename DoubleType> void set_values(const DoubleType & /*v*/);
