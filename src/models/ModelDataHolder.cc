...
template <> const double &ModelDataHolder::GetUniformValue() {
  return double_uniform_value;
}

#ifdef DEVSIM_EXTENDED_PRECISION
template <> const float128 &ModelDataHolder::GetUniformValue() {
  return float128_uniform_value;
}
#endif

template <> ModelDataHolder::MDtype ModelDataHolder::GetMDtype<double>() const {
...
template <> const std::vector<double> &ModelDataHolder::GetValues() const {
  expand_uniform();
#ifdef DEVSIM_EXTENDED_PRECISION
...
#ifdef DEVSIM_EXTENDED_PRECISION
template <> const std::vector<float128> &ModelDataHolder::GetValues() const {
  expand_uniform();
...
template <> void ModelDataHolder::set_values(const std::vector<double> &nv) {
  clear_type(MDtype::EXTENDED);
  type = MDtype::DOUBLE;
  double_values = nv;
  is_uniform = false;
...
#ifdef DEVSIM_EXTENDED_PRECISION
template <> void ModelDataHolder::set_values(const std::vector<float128> &nv) {
  clear_type(MDtype::DOUBLE);
  type = MDtype::EXTENDED;
  float128_values = nv;
  is_uniform = false;
...
#ifdef DEVSIM_EXTENDED_PRECISION
template <> void ModelDataHolder::SetValue(size_t index, const float128 &nv) {
  if (index >= length) {
    return;
  }
...
template <> void ModelDataHolder::SetValue(size_t index, const double &nv) {
  if (index >= length) {
    return;
  }
