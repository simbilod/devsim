...
ScalarData<T, DoubleType>::ScalarData(const ScalarData<T, DoubleType> &em)
    : refdata(em.refdata), values(em.values), isuniform(em.isuniform),
      uniform_value(em.uniform_value), length(em.length) {}

template <typename T, typename DoubleType>
ScalarData<T, DoubleType> &
ScalarData<T, DoubleType>::operator=(const ScalarData &em) {
  if (this != &em) {
    refdata = em.refdata;
    values = em.values;
...
ScalarData<T, DoubleType>::GetScalarList() const {
...
template <typename V>
ScalarData<T, DoubleType> &
ScalarData<T, DoubleType>::op_equal_model(const T &nm, const V &myop) {
...
template <typename V>
ScalarData<T, DoubleType> &
ScalarData<T, DoubleType>::op_equal_data(const ScalarData<T, DoubleType> &esd,
                                         const V &myop) {
...
template <typename V>
ScalarData<T, DoubleType> &
ScalarData<T, DoubleType>::op_equal_scalar(const DoubleType &v, const V &myop) {
  if (isuniform) {
...
template <typename T, typename DoubleType>
ScalarData<T, DoubleType> &
ScalarData<T, DoubleType>::times_equal_model(const T &nm) {
...
template <typename T, typename DoubleType>
ScalarData<T, DoubleType> &
ScalarData<T, DoubleType>::times_equal_scalar(DoubleType x) {
...
template <typename T, typename DoubleType>
ScalarData<T, DoubleType> &
ScalarData<T, DoubleType>::plus_equal_model(const T &nm) {
  if (this->IsZero()) {
    *this = ScalarData<T, DoubleType>(nm);
  } else {
...
template <typename T, typename DoubleType>
ScalarData<T, DoubleType> &
ScalarData<T, DoubleType>::plus_equal_scalar(DoubleType x) {
  if (x == 0.0) {
    // do nothing
  } else if (this->IsZero()) {
