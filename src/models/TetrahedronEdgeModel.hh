...
using WeakConstTetrahedronEdgeModelPtr =
    std::weak_ptr<const TetrahedronEdgeModel>;
...
public:
  ...
  TetrahedronEdgeModel(const std::string &, const RegionPtr,
  ...
  template <typename DoubleType>
  void GetScalarValuesOnNodes(TetrahedronEdgeModel::InterpolationType,
  ...
  template <typename DoubleType> void SetValues(const DoubleType &);
  ...
  ConstTetrahedronEdgeModelPtr GetConstSelfPtr() const { return myself.lock(); }

  TetrahedronEdgeModelPtr GetSelfPtr() { return myself.lock(); }

  bool IsUniform() const;

  template <typename DoubleType> const DoubleType &GetUniformValue() const;
...
protected:
  ...
  template <typename DoubleType> void SetValues(const DoubleType &) const;
...
TetrahedronEdgeModelPtr create_tetrahedron_edge_model(bool use_extended,
                                                      Args &&...args) {
