...
TetrahedronEdgeFromEdgeModel<DoubleType>::TetrahedronEdgeFromEdgeModel(
    const std::string &edgemodel, RegionPtr rp)
    : TetrahedronEdgeModel(edgemodel + "_x", rp,
                           TetrahedronEdgeModel::DisplayType::SCALAR),
      edgeModelName(edgemodel), y_ModelName(edgeModelName + "_y"),
      z_ModelName(edgeModelName + "_z") {
...
template <typename DoubleType>
void TetrahedronEdgeFromEdgeModel<DoubleType>::calcTetrahedronEdgeScalarValues()
    const {
...
template <typename DoubleType>
void TetrahedronEdgeFromEdgeModel<DoubleType>::Serialize(
...
CreateTetrahedronEdgeFromEdgeModel(const std::string &edgemodel, RegionPtr rp) {
  const bool use_extended = rp->UseExtendedPrecisionModels();
  return create_tetrahedron_edge_model<
      TetrahedronEdgeFromEdgeModel<double>,
      TetrahedronEdgeFromEdgeModel<extended_type>>(use_extended, edgemodel, rp);
}
