...
TriangleEdgeFromEdgeModel<DoubleType>::TriangleEdgeFromEdgeModel(
...
template <typename DoubleType>
void TriangleEdgeFromEdgeModel<DoubleType>::calcTriangleEdgeScalarValues()
    const {
...
CreateTriangleEdgeFromEdgeModel(const std::string &edgemodel, RegionPtr rp) {
  const bool use_extended = rp->UseExtendedPrecisionModels();
  return create_triangle_edge_model<TriangleEdgeFromEdgeModel<double>,
                                    TriangleEdgeFromEdgeModel<extended_type>>(
