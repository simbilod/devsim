...
const char *TetrahedronEdgeModel::DisplayTypeString[] = {"nodisplay", "scalar",
                                                         nullptr};
...
TetrahedronEdgeModel::TetrahedronEdgeModel(const std::string &nm,
                                           const RegionPtr rp,
                                           TetrahedronEdgeModel::DisplayType dt)
    : name(nm), myregion(rp), uptodate(false), inprocess(false),
      displayType(dt), model_data(rp->GetNumberTetrahedrons() * 6) {
...
const TetrahedronEdgeScalarList<DoubleType> &
TetrahedronEdgeModel::GetScalarValues() const {
  CalculateValues();
...
template <typename DoubleType>
void TetrahedronEdgeModel::SetValues(
    const TetrahedronEdgeScalarList<DoubleType> &nv) {
...
template <typename DoubleType>
void TetrahedronEdgeModel::SetValues(
    const TetrahedronEdgeScalarList<DoubleType> &nv) const {
...
template <typename DoubleType>
void TetrahedronEdgeModel::GetScalarValuesOnNodes(
    TetrahedronEdgeModel::InterpolationType interpolation_type,
    std::vector<DoubleType> &values) const {
  const Region &region = this->GetRegion();

  const TetrahedronEdgeScalarList<DoubleType> &esl =
  ...
  const Region::TetrahedronToConstEdgeDataList_t &ttcedl =
      region.GetTetrahedronToEdgeDataList();

  if ((interpolation_type ==
       TetrahedronEdgeModel::InterpolationType::AVERAGE) ||
      (interpolation_type == TetrahedronEdgeModel::InterpolationType::SUM)) {
      ...
      for (size_t i = 0; i < number_nodes; ++i) {
        ...
        if (c != 0) {
          values[i] /= c;
             ...
             TetrahedronEdgeModel::InterpolationType::COUPLE) {
    ConstTetrahedronEdgeModelPtr couple_ptr =
        region.GetTetrahedronEdgeModel("ElementEdgeCouple");
    dsAssert(couple_ptr.get(), "UNEXPECTED");
    const TetrahedronEdgeScalarList<DoubleType> &element_edge_couples =
...
template <typename DoubleType>
void TetrahedronEdgeModel::GetScalarValuesOnElements(
  ...
  const DoubleType scale =
      static_cast<DoubleType>(1.0) / static_cast<DoubleType>(6.0);
  size_t mindex = 0;
  for (size_t i = 0; i < number_tetrahedron; ++i) {
...
EdgeScalarList<DoubleType> TetrahedronEdgeModel::GetValuesOnEdges() const {
  const TetrahedronEdgeScalarData<DoubleType> &tec =
      TetrahedronEdgeScalarData<DoubleType>(*this);
  ...
  for (size_t i = 0; i < edge_list.size(); ++i) {
    ...
    for (; it != itend; ++it) {
      ...
      const size_t eindex =
          GetRegion().GetEdgeIndexOnTetrahedron(**it, edge_list[i]);
