...
template <typename DoubleType>
EdgeFromNodeModel<DoubleType>::EdgeFromNodeModel(const std::string &edgemodel0,
                                                 const std::string &edgemodel1,
                                                 const std::string &nodemodel,
                                                 const RegionPtr rp)
    : EdgeModel(edgemodel0, rp,
                EdgeModel::DisplayType::SCALAR), // The name of this node model
      nodeModelName(nodemodel), edgeModel1Name(edgemodel1) {
  // The other model is going to be set from our model
  //// TODO: issues, use soft references by name here, just like we do other
  ///node models
  node1EdgeModel = EdgeSubModel<DoubleType>::CreateEdgeSubModel(
      edgemodel1, rp, EdgeModel::DisplayType::SCALAR, this->GetSelfPtr());
...
void EdgeFromNodeModel<DoubleType>::Serialize(std::ostream &of) const {
  of << "COMMAND edge_from_node_model -device \"" << GetDeviceName()
...
EdgeModelPtr CreateEdgeFromNodeModel(const std::string &edgemodel0,
                                     const std::string &edgemodel1,
                                     const std::string &nodemodel,
                                     const RegionPtr rp) {
  const bool use_extended = rp->UseExtendedPrecisionModels();
  return create_edge_model<EdgeFromNodeModel<double>,
                           EdgeFromNodeModel<extended_type>>(
      use_extended, edgemodel0, edgemodel1, nodemodel, rp);
}
