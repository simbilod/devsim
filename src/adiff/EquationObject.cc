...
UnaryTblEntry UnaryTable[] = {
    ...
    {"expand", Expand,
     "expand(obj)          -- Perform expansion of products with sums"},
    {"log", log, "log(obj)              -- Natural logarithm"},
    {"unscaledval", getUnscaledValue,
     "unscaledval(obj)      -- Get value without constant scaling"},
    {"scale", getConstantFactor,
     "scale(obj)            -- Get constant factor"},
    {"unsignedval", getUnsignedValue,
...
BinaryTblEntry BinaryTable[] = {
    ...
    {"diff", diff,
     "diff(obj1, obj2)      -- Take derivative  of obj1 with respect to obj2"},
    {nullptr, nullptr, nullptr}};
...
Variable::Variable(std::string var)
    : EquationObject(VARIABLE_OBJ), value(var) {}

std::string Variable::createStringValue() { return value; }
...
Pow::Pow(EqObjPtr b, EqObjPtr e)
    : EquationObject(POW_OBJ), base(b), exponent(e) {}
...
std::set<std::string> UniteReferencedType(EqObjType rt, const EqObjPtr &op1,
                                          const EqObjPtr &op2) {
...
std::set<std::string> UniteReferencedType(EqObjType rt,
