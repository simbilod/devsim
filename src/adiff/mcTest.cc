...
// https://connect.microsoft.com/VisualStudio/feedback/details/1368280
  ...
  */
  ...
  dsAssert(EquationToNodeMap.size() ==
               (ExternalNodeList.size() + InternalNodeList.size()),
           "UNEXPECTED");
...
std::string parameterListName(const std::string &x) { return x; }
...
public:
  ...
  Eqo::EqObjPtr getEquation() const { return equation->clone(); }
...
void createSubexpressions() {
  ...
  typedef std::list<std::pair<std::string, Eqo::EqObjPtr>>::iterator
      StrEqListIt;
  ...
  for (size_t i = 0; i < final_list.size(); ++i) {
    ...
    ModelList.push_back(
        std::make_pair(subexpr.getName(), subexpr.getEquation()));
  }
  semit = ModelList.begin();
  semend = ModelList.end();
  for (; semit != semend; ++semit) {
    for (size_t i = 0; i < final_list.size(); ++i) {
      ...
      if (subexpr.getName() != semit->first) {
        semit->second =
            (semit->second)->subst(final_list[i], Eqo::mod(subexpr.getName()));
      }
    }
  }
}
...
*/
std::string printForEquation(std::map<std::string, bool> &wasProcessed,
                             Eqo::EqObjPtr &eq) {
  ostringstream out;

  //// It this equation is only a reference to a model, and the string was
  ///already printout out, / then just resturn an empty string
  ...
  if (eq->getType() == Eqo::MODEL_OBJ) {
    const std::string mname = eq->stringValue();
    out << "   const double " << eq << " = " << findInModelList(mname) << ";\n";
...
void GenerateEquationDerivatives() {
  typedef std::map<std::string,
                   std::pair<Eqo::EqObjPtr, Eqo::EqObjPtr>>::iterator
      StrEqEqMapIt;
  ...
  for (; seemit != seemend; ++seemit) {
    ...
    for (; sit != send; ++sit) {
      ...
      std::string tstr =
          Eqo::Simplify(Eqo::diff((*seemit).second.first, Eqo::var(*sit)))
              ->stringValue();
      cerr << "  dc," << (*sit);
      cerr << "  " << tstr << endl;

      tstr = Eqo::Simplify(Eqo::diff((*seemit).second.second, Eqo::var(*sit)))
                 ->stringValue();
      cerr << "  td," << (*sit);
      cerr << "  " << tstr << endl;
    }
...
void print_hh(ofstream &out) {
  ...
  */
  ...
  out << "\n"
      ...
      << ClassName
      << "_create"
         " (NodeKeeper *, const std::string &name, const "
  ...
  */
  ...
  out << ");\n\n";
  out << "       void assembleDC_impl(const NodeKeeper::Solution &, "
         "dsMath::RealRowColValueVec<double> &, std::vector<std::pair<int, "
         "double> > &);\n"

         "       void assembleTran_impl(const double scl, const "
         "NodeKeeper::Solution &sol, dsMath::RealRowColValueVec<double> *mat, "
  ...
  out << "       //External Nodes\n";
  for (lit = ExternalNodeList.begin(); lit != ExternalNodeList.end(); ++lit) {
    out << "       CircuitNodePtr " << nodePtrName(*lit) << ";\n";
  }
  out << "\n"
         "       //Internal Nodes\n";
  ...
  out << "\n"
         "       //Parameter List\n";
  typedef std::map<std::string, std::pair<std::string, double>>::iterator
      Plistit;
  {
    std::ostringstream os;
    for (Plistit it = ParameterList.begin(); it != ParameterList.end(); ++it) {
      os << "       double " << std::setw(15) << std::left
         << parameterListName(it->first) + ";"
         << "//" << std::setw(20) << it->second.first << " " << std::scientific
         << it->second.second << "\n";
    }
    out << os.str();
  }
...
void print_cc(ofstream &out, const string &headerFile) {
  ...
  i = 1;
  for (lit = ExternalNodeList.begin(); lit != ExternalNodeList.end(); ++lit) {
    out << "       " << nodePtrName(*lit) << "= this->AddCircuitNode(n" << i
        << ");\n";
    ++i;
  }
  for (lit = InternalNodeList.begin(); lit != InternalNodeList.end(); ++lit) {
    out << "       " << nodePtrName(*lit) << "= this->AddInternalNode(\""
        << *lit << "\");\n";
    ++i;
  }
      ...
      Plistit;
  {
    ...
    for (Plistit it = ParameterList.begin(); it != ParameterList.end(); ++it) {
      os << "       " << std::setw(15) << std::left
         << parameterListName(it->first) << " = " << std::scientific
         << it->second.second << ";\n";
    }
    out << os.str();
  }
  ...
  out << "bool " << ClassName
      << "::addParam(const std::string &str, double val)\n"
         "{\n";
  //   typedef std::map<std::string, std::pair<std::string, double> >::iterator
  //   Plistit;
  out << "    bool ret = false;\n";
  ...
  /// Make this optional compilation for user models
  out << "\nextern \"C\" InstanceModel *" << ClassName
      << "_create"
         " (NodeKeeper *nk, const std::string &name, const "
         "std::vector<std::string> &nodelist)"
         " {\n"
         " return new "
      << ClassName << "(nk, name.c_str()";
...
void PrintAssemblyRoutine(ofstream &out, assembletype_t atype) {
  ...
  if (atype == DCASSEMBLE) {
    out << "void " << ClassName
        << "::assembleDC_impl(const NodeKeeper::Solution &sol, "
           "dsMath::RealRowColValueVec<double> &mat, "
           "std::vector<std::pair<int, double> > &rhs)\n";
  } else if (atype == ACASSEMBLE) {
    out << "void " << ClassName
        << "::assembleTran_impl(const double scl, const NodeKeeper::Solution "
           "&sol, dsMath::RealRowColValueVec<double> *mat, "
           "std::vector<std::pair<int, double> > &rhs)\n";
  }
  ...
  for (seemit = EquationList.begin(); seemit != EquationList.end(); ++seemit) {
    ...
    bos[EQNNUMBER] << "   const size_t " << nodeNbrName(tmpname) << " = "
                   << nodePtrName(tmpname) << "->getNumber();\n";

    bos[ISGROUND] << "   const bool   " << nodeIsGndName(tmpname) << " = "
                  << nodePtrName(tmpname) << "->isGROUND();\n";

    bos[SOLUTION] << "   const double " << tmpname << " = ("
                  << nodeIsGndName(tmpname)
                  << ") ? 0.0 : "
                     "sol["
                  << nodeNbrName(tmpname) << "];\n";
    ...
    bos[RHSEVAL] << "   if (!" << nodeIsGndName(tmpname)
                 << ")\n"
                    "      rhs.push_back(std::make_pair("
                 << nodeNbrName(tmpname) << ", " << seemit->first << "));\n";

    bos[MATRIX] << "   if (!" << nodeIsGndName(tmpname)
                << ")\n"
                   "   {\n";
    ...
    for (lit = TotalNodeList.begin(); lit != TotalNodeList.end(); ++lit) {
      ...
      if (tmpname != *lit) {
        bos[MATRIX] << "      if (!" << nodeIsGndName(*lit)
                    << ")\n"
                       "   ";
      }

      bos[MATRIX] << "      mat" << op1
                  << "push_back(dsMath::RealRowColVal<double>("
                  << nodeNbrName(tmpname) << "," << nodeNbrName(*lit) << ", "
                  << nm << "));\n";
    }
    bos[MATRIX] << "   }\n";
  }
