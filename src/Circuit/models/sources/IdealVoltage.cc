...
IdealVoltage::IdealVoltage(NodeKeeper *nkp,
...
IdealVoltage::IdealVoltage(NodeKeeper *nkp,
                           ...
                           const char *n1, const char *n2,
                           double value, // DV voltage
                           double acrvalue, double acivalue,
                           const SignalPtr sig)
...
void IdealVoltage::assembleDC_impl(const NodeKeeper::Solution &sol,
                                   dsMath::RealRowColValueVec<double> &vec,
...
void IdealVoltage::assembleACRHS_impl(
    std::vector<std::pair<size_t, std::complex<double>>> &x) {
  const int rI = nI_->getNumber();
