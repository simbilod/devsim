...
DCOnly::DCOnly(double x) : Signal(SignalType::DCONLY) { _volt = x; }

double DCOnly::NextBreakPoint(double) { return DBL_MAX; }

double DCOnly::CalcVoltage(double) { return _volt; }

Pulse::Pulse(double v1, double v2, double td, double tr, double tf, double pw,
             double per)
    : Signal(SignalType::PULSE), // add sanity checks later
      _v1(v1), _v2(v2), _td(td), _tr(tr), _tf(tf), _pw(pw), _per(per) {
  _volt = v1;
}
