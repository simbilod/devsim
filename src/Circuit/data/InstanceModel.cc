...
InstanceModel::InstanceModel(NodeKeeper *nkp, const char *name)
    : nodekeep_(nkp), name_(name) {}
...
/**
  MNA nodes are ones with no entry on their diagonal.  Right now, this is not an
  issue. It also creates a prefix to the name.
  */
...
void InstanceModel::assembleDC(const std::vector<double> &sol,
                               dsMath::RealRowColValueVec<double> &mat,
                               dsMath::RHSEntryVec<double> &rhs) {
  this->assembleDC_impl(sol, mat, rhs);
}

void InstanceModel::assembleTran(const double scl,
...
void InstanceModel::assembleACRHS(
