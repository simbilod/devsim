 ...
 * and a default type
 */
CircuitNodePtr NodeKeeper::AddNode(const std::string &name, CircuitNodeType nt,
                                   CircuitUpdateType ut) {
  //    os << "NodeKeeper::AddNode\n";

  if (SolutionLocked_) {
    std::ostringstream os;
    os << "CircuitNode " << name
       << " cannot be added once an analysis has been performed.\n";
...
CircuitNodePtr NodeKeeper::AddNodeAlias(const std::string &alias,
  ...
  if (nalias != NodeTable_.end()) {
    std::ostringstream os;
    os << "CircuitNode " << alias
       << " cannot be an alias since it is already a real node.\n";
    OutputStream::WriteOut(OutputStream::OutputType::FATAL, os.str());
    return nullptr;
  }
...
void NodeKeeper::SetNodeNumbers(size_t start, bool verbose) {
  ...
  for (iter = NodeTable_.begin(); iter != end; ++iter) {
    ...
    if (verbose) {
      os << iter->first << "\t" << iter->second->GetNumber() << "\t"
         << iter->second->getCircuitNodeTypeStr() << "\n";
    }
  }
  if (verbose) {
    OutputStream::WriteOut(OutputStream::OutputType::INFO, os.str());
 ...
 */
const NodeKeeper::Solution &
NodeKeeper::GetSolutionVector(const std::string &key) {
...
void NodeKeeper::UpdateSolution(const std::string &key, const Solution &rhs) {
  ...
  for (NodeTable_t::iterator it = NodeTable_.begin(); it != NodeTable_.end();
       ++it) {
    CircuitNode &cn = *(it->second);
    if (cn.isGROUND()) {
      continue;
...
void NodeKeeper::ACUpdateSolution(const std::string &rkey,
                                  const std::string &ikey,
                                  const ACSolution &rhs) {
...
bool NodeKeeper::HaveNodes() { return NodeTable_.size() != 0; }
...
double NodeKeeper::GetNodeValue(const std::string &solutionname,
...
void NodeKeeper::SetNodeValue(const std::string &solutionname,
                              const std::string &nodename, double value) {
  if (Sol_.count(solutionname)) {
    size_t index = GetIndexNumber(nodename);
    if (index != size_t(-1)) {
      dsAssert(index < (*Sol_[solutionname]).size(), "CIRCUIT_UNEXPECTED");
...
void NodeKeeper::TriggerCallbacksOnNodes() {
  for (NodeTable_t::const_iterator it = NodeTable_.begin();
       it != NodeTable_.end(); ++it) {
    TriggerCallbackOnNode(it->first);
  }
  for (NodeAliasTable_t::const_iterator it = NodeAliasTable_.begin();
       it != NodeAliasTable_.end(); ++it) {
    TriggerCallbackOnNode(it->first);
  }
}
