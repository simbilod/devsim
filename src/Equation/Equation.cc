...
template <typename DoubleType>
Equation<DoubleType>::Equation(const std::string &nm, RegionPtr rp,
                               const std::string &var,
                               EquationEnum::UpdateType ut)
    : myname(nm), myregion(rp), variable(var), absError(0.0), relError(0.0),
      absErrorNodeIndex(0), relErrorNodeIndex(0), minError(defminError),
      updateType(ut) {
  EquationHolder ptr(this);
  rp->AddEquation(ptr);
}

template <typename DoubleType> Equation<DoubleType>::~Equation() {}

// At this level, we can probably control which row we wish to assemble
template <typename DoubleType>
void Equation<DoubleType>::Assemble(dsMath::RealRowColValueVec<DoubleType> &m,
...
template <typename DoubleType>
void Equation<DoubleType>::Update(NodeModel &nm,
                                  const dsMath::DoubleVec_t<DoubleType> &rhs) {
  UpdateValues(nm, rhs);
}

template <typename DoubleType>
void Equation<DoubleType>::ACUpdate(
    NodeModel &nm, const dsMath::ComplexDoubleVec_t<DoubleType> &rhs) {
  ACUpdateValues(nm, rhs);
}

template <typename DoubleType>
void Equation<DoubleType>::NoiseUpdate(
    const std::string &nm, const std::vector<PermutationEntry> &permvec,
    const dsMath::ComplexDoubleVec_t<DoubleType> &rhs) {
  NoiseUpdateValues(nm, permvec, rhs);
}

template <typename DoubleType>
void Equation<DoubleType>::LogSolutionUpdate(
    const NodeScalarList<DoubleType> &ovals, NodeScalarList<DoubleType> &upds,
    NodeScalarList<DoubleType> &nvals) {
...
template <typename DoubleType>
void Equation<DoubleType>::PositiveSolutionUpdate(
    const NodeScalarList<DoubleType> &ovals, NodeScalarList<DoubleType> &upds,
    NodeScalarList<DoubleType> &nvals) {
  for (size_t i = 0; i < ovals.size(); ++i) {
    const DoubleType oval = ovals[i];
    ...
    if (oval > 0) {
      ...
      dsErrors::SolutionVariableNonPositive(reg, myname, GetVariable(), oval,
...
template <typename DoubleType>
void Equation<DoubleType>::DefaultSolutionUpdate(
    const NodeScalarList<DoubleType> &ovals, NodeScalarList<DoubleType> &upds,
    NodeScalarList<DoubleType> &nvals) {
  for (size_t i = 0; i < ovals.size(); ++i) {
...
template <typename DoubleType>
void Equation<DoubleType>::DefaultUpdate(
    NodeModel &nm, const dsMath::DoubleVec_t<DoubleType> &result) {
  const Region &reg = *myregion;

  const size_t ind = reg.GetEquationIndex(myname);
  //    dsAssert(ind != size_t(-1), "UNEXPECTED");
  if (ind == size_t(-1)) {
    dsErrors::MissingEquationIndex(reg, myname, "",
                                   OutputStream::OutputType::FATAL);
    return;
  }
  ...
  //    dsErrors::EquationMathErrorInfo(*this, rerr, aerr,
  //    OutputStream::OutputType::INFO);
}

//// TODO: Make sure noise and ac kept in sync
template <typename DoubleType>
void Equation<DoubleType>::DefaultACUpdate(
    NodeModel &nm, const dsMath::ComplexDoubleVec_t<DoubleType> &result) {
  ...
  //    dsAssert(ind != size_t(-1), "UNEXPECTED");
  if (ind == size_t(-1)) {
    dsErrors::MissingEquationIndex(*myregion, myname, "",
                                   OutputStream::OutputType::FATAL);
    return;
  }
  ...
  NodeModelPtr rnm = std::const_pointer_cast<NodeModel, const NodeModel>(
      myregion->GetNodeModel(realnodemodel));
  NodeModelPtr inm = std::const_pointer_cast<NodeModel, const NodeModel>(
      myregion->GetNodeModel(imagnodemodel));

  if (!rnm) {
    dsErrors::CreateModelOnRegion(*myregion, realnodemodel,
                                  OutputStream::OutputType::INFO);
    rnm = CreateNodeSolution(realnodemodel, myregion,
                             NodeModel::DisplayType::SCALAR);
  }

  if (!inm) {
    dsErrors::CreateModelOnRegion(*myregion, imagnodemodel,
                                  OutputStream::OutputType::INFO);
...
template <typename DoubleType>
void Equation<DoubleType>::DefaultNoiseUpdate(
    const std::string &outputname, const std::vector<PermutationEntry> &permvec,
  ...
  const ConstNodeList::const_iterator nend = nl.end();
  for (; nit != nend; ++nit) {
    // TODO: we need to make sure we handle when the equation was copied,
    // KeepCopy() == true
    const size_t eqrow =
        permvec[myregion->GetEquationNumber(ind, *nit)].GetRow();
  ...
  if (!myregion->GetNodeModel(xrname)) {
    CreateVectorGradient(myregion, realnodemodel,
                         VectorGradientEnum::AVOIDZERO);
  }
  if (!myregion->GetNodeModel(xiname)) {
    CreateVectorGradient(myregion, imagnodemodel,
                         VectorGradientEnum::AVOIDZERO);
  }
}

template <typename DoubleType>
void Equation<DoubleType>::EdgeAssembleRHS(
    dsMath::RHSEntryVec<DoubleType> &v, const EdgeScalarData<DoubleType> &eflux,
    const DoubleType n0_sign, const DoubleType n1_sign) {
  ...
  if (eqindex0 == size_t(-1)) {
    dsErrors::MissingEquationIndex(r, myname, "",
                                   OutputStream::OutputType::FATAL);
    return;
  }
...
template <typename DoubleType>
void Equation<DoubleType>::TriangleEdgeAssembleRHS(
    dsMath::RHSEntryVec<DoubleType> &v,
    const TriangleEdgeScalarData<DoubleType> &teflux, const DoubleType n0_sign,
...
template <typename DoubleType>
void Equation<DoubleType>::TetrahedronEdgeAssembleRHS(
    dsMath::RHSEntryVec<DoubleType> &v,
    const TetrahedronEdgeScalarData<DoubleType> &teflux,
    const DoubleType n0_sign, const DoubleType n1_sign) {
  const Region &r = GetRegion();
  const size_t eqindex0 = r.GetEquationIndex(myname);
  //  dsAssert(eqindex0 != size_t(-1), "UNEXPECTED");
  ...
  const Region::TetrahedronToConstEdgeDataList_t &ttelist =
      r.GetTetrahedronToEdgeDataList();
  for (size_t i = 0; i < ttelist.size(); ++i) {
...
/// Inserts the entries into the matrix for the case when the sensitivity is
/// different with respect to the same variable at the nodes opposite of the
/// edge eder0 is derivative wrt first node eder1 is derivative wrt second node
template <typename DoubleType>
void Equation<DoubleType>::UnSymmetricEdgeAssembleJacobian(
    dsMath::RealRowColValueVec<DoubleType> &m,
    const EdgeScalarData<DoubleType> &eder0,
    const EdgeScalarData<DoubleType> &eder1, const std::string &var,
    const DoubleType n0_sign, const DoubleType n1_sign) {
  const Region &r = GetRegion();
  const size_t eqindex0 = r.GetEquationIndex(myname);
  //    dsAssert(eqindex0 != size_t(-1), "UNEXPECTED");
  ...
  const size_t eqindex1 =
      r.GetEquationIndex(r.GetEquationNameFromVariable(var));
  //    dsAssert(eqindex1 != size_t(-1), "UNEXPECTED");
  if (eqindex1 == size_t(-1)) {
    dsErrors::MissingEquationIndex(r, myname, var,
                                   OutputStream::OutputType::FATAL);
    return;
  }
  ...
  for (size_t i = 0; i < el.size(); ++i) {
    ...
    m.push_back(
        dsMath::RealRowColVal<DoubleType>(row0, col0, n0_sign * ederval0));
    m.push_back(
        dsMath::RealRowColVal<DoubleType>(row1, col1, n1_sign * ederval1));
    m.push_back(
        dsMath::RealRowColVal<DoubleType>(row0, col1, n0_sign * ederval1));
    m.push_back(
        dsMath::RealRowColVal<DoubleType>(row1, col0, n1_sign * ederval0));
  }
}
...
template <typename DoubleType>
void Equation<DoubleType>::UnSymmetricTriangleEdgeAssembleJacobian(
    dsMath::RealRowColValueVec<DoubleType> &m,
    const TriangleEdgeScalarData<DoubleType> &eder0,
    const TriangleEdgeScalarData<DoubleType> &eder1,
    const TriangleEdgeScalarData<DoubleType> &eder2, const std::string &var,
    const DoubleType n0_sign, const DoubleType n1_sign) {
  const Region &r = GetRegion();
  const size_t eqindex0 = r.GetEquationIndex(myname);
  //  dsAssert(eqindex0 != size_t(-1), "UNEXPECTED");
  ...
  for (size_t i = 0; i < ttelist.size(); ++i) {
    ...
    for (size_t j = 0; j < el.size(); ++j) {
      ...
      /// Here we account for the fact stuff moving toward row1 has opposite
      /// sign
      m.push_back(
          dsMath::RealRowColVal<DoubleType>(row0, col0, n0_sign * ederval0));
      m.push_back(
      ...
      m.push_back(
          dsMath::RealRowColVal<DoubleType>(row0, col2, n0_sign * ederval2));
      m.push_back(
          dsMath::RealRowColVal<DoubleType>(row1, col2, n1_sign * ederval2));
    }
  }
}

template <typename DoubleType>
void Equation<DoubleType>::UnSymmetricTetrahedronEdgeAssembleJacobian(
    dsMath::RealRowColValueVec<DoubleType> &m,
    const TetrahedronEdgeScalarData<DoubleType> &eder0,
    const TetrahedronEdgeScalarData<DoubleType> &eder1,
    const TetrahedronEdgeScalarData<DoubleType> &eder2,
    const TetrahedronEdgeScalarData<DoubleType> &eder3, const std::string &var,
    const DoubleType n0_sign, const DoubleType n1_sign) {
  const Region &r = GetRegion();
  const size_t eqindex0 = r.GetEquationIndex(myname);
  //  dsAssert(eqindex0 != size_t(-1), "UNEXPECTED");
  ...
  for (size_t i = 0; i < ttelist.size(); ++i) {
    ...
    for (size_t j = 0; j < edgeDataList.size(); ++j) {
      ...
      m.push_back(
          dsMath::RealRowColVal<DoubleType>(row1, col2, n1_sign * ederval2));
      m.push_back(
          dsMath::RealRowColVal<DoubleType>(row0, col3, n0_sign * ederval3));
      m.push_back(
          dsMath::RealRowColVal<DoubleType>(row1, col3, n1_sign * ederval3));
    }
  }
}
...
template <typename DoubleType>
void Equation<DoubleType>::NodeAssembleRHS(
    dsMath::RHSEntryVec<DoubleType> &v,
    const NodeScalarData<DoubleType> &nrhs) {
  // var is the name of the variable we are differentiating with respect to.  It
  // is therefore the column and our equation is the row
...
template <typename DoubleType>
void Equation<DoubleType>::NodeAssembleJacobian(
    dsMath::RealRowColValueVec<DoubleType> &m,
    const NodeScalarData<DoubleType> &nder, const std::string &var) {
  // var is the name of the variable we are differentiating with respect to.  It
  // is therefore the column and our equation is the row
...
template <typename DoubleType>
void Equation<DoubleType>::NodeVolumeAssemble(
    const std::string &model, dsMath::RealRowColValueVec<DoubleType> &m,
    dsMath::RHSEntryVec<DoubleType> &v, dsMathEnum::WhatToLoad w) {
...
void Equation<DoubleType>::NodeVolumeAssemble(
  ...
  if (!nm) {
    dsErrors::MissingEquationModel(r, myname, model, dsErrors::ModelInfo::NODE,
                                   OutputStream::OutputType::FATAL);
    return;
  }
  ...
  if (!nvol) {
    dsErrors::MissingEquationModel(r, myname, node_volume,
                                   dsErrors::ModelInfo::NODE,
                                   OutputStream::OutputType::FATAL);
    return;
  }

  if ((w == dsMathEnum::WhatToLoad::RHS) ||
      (w == dsMathEnum::WhatToLoad::MATRIXANDRHS)) {
  ...
  if ((w == dsMathEnum::WhatToLoad::MATRIXONLY) ||
    ...
    for (; it != vlist.end(); ++it) {
      ...
      if (!ndm) {
        dsErrors::MissingEquationModel(r, myname, dermodel,
                                       dsErrors::ModelInfo::NODE,
                                       OutputStream::OutputType::VERBOSE1);
      } else {
...
template <typename DoubleType>
void Equation<DoubleType>::EdgeCoupleAssemble(
    const std::string &model, dsMath::RealRowColValueVec<DoubleType> &m,
    dsMath::RHSEntryVec<DoubleType> &v, dsMathEnum::WhatToLoad w) {
...
template <typename DoubleType>
void Equation<DoubleType>::EdgeNodeVolumeAssemble(
    const std::string &model, dsMath::RealRowColValueVec<DoubleType> &m,
    dsMath::RHSEntryVec<DoubleType> &v, dsMathEnum::WhatToLoad w) {
...
void Equation<DoubleType>::EdgeCoupleAssemble(
    const std::string &model, dsMath::RealRowColValueVec<DoubleType> &m,
    dsMath::RHSEntryVec<DoubleType> &v, dsMathEnum::WhatToLoad w,
    const std::string &edge_couple, const DoubleType n0_sign,
    const DoubleType n1_sign) {
  const Region &r = GetRegion();
  ...
  if (!ef) {
    dsErrors::MissingEquationModel(r, myname, model, dsErrors::ModelInfo::EDGE,
                                   OutputStream::OutputType::FATAL);
    return;
  }
  ...
  if (!ec) {
    dsErrors::MissingEquationModel(r, myname, edge_couple,
                                   dsErrors::ModelInfo::EDGE,
                                   OutputStream::OutputType::FATAL);
    return;
  }
  ...
  if ((w == dsMathEnum::WhatToLoad::MATRIXONLY) ||
    ...
    for (; it != vlist.end(); ++it) {
      ...
      if ((!em0) && (!em1)) {
        dsErrors::MissingEquationModel(r, myname, dermodel0,
                                       dsErrors::ModelInfo::EDGE,
                                       OutputStream::OutputType::VERBOSE1);
        dsErrors::MissingEquationModel(r, myname, dermodel1,
                                       dsErrors::ModelInfo::EDGE,
                                       OutputStream::OutputType::VERBOSE1);
      } else {
...
template <typename DoubleType>
void Equation<DoubleType>::TriangleEdgeCoupleAssemble(
    const std::string &model, dsMath::RealRowColValueVec<DoubleType> &m,
    dsMath::RHSEntryVec<DoubleType> &v, dsMathEnum::WhatToLoad w,
    const std::string &edge_couple, const DoubleType n0_sign,
    const DoubleType n1_sign) {
  ...
  if (!ef) {
    dsErrors::MissingEquationModel(r, myname, model,
                                   dsErrors::ModelInfo::ELEMENTEDGE,
                                   OutputStream::OutputType::FATAL);
    return;
  }
  ...
  if ((w == dsMathEnum::WhatToLoad::RHS) ||
      (w == dsMathEnum::WhatToLoad::MATRIXANDRHS)) {
    TriangleEdgeScalarData<DoubleType> eflux =
        TriangleEdgeScalarData<DoubleType>(*ef);
    eflux.times_equal_model(*ec);
    ...
    for (; it != vlist.end(); ++it) {
      ...
      if ((!em0) && (!em1) && (!em2)) {
        ...
        dsErrors::MissingEquationModel(r, myname, dermodel1,
                                       dsErrors::ModelInfo::ELEMENTEDGE,
                                       OutputStream::OutputType::VERBOSE1);
        dsErrors::MissingEquationModel(r, myname, dermodel2,
                                       dsErrors::ModelInfo::ELEMENTEDGE,
                                       OutputStream::OutputType::VERBOSE1);
      } else {
        if (!em0) {
        ...
        TriangleEdgeScalarData<DoubleType> eder0 =
            TriangleEdgeScalarData<DoubleType>(*em0);
        TriangleEdgeScalarData<DoubleType> eder1 =
            TriangleEdgeScalarData<DoubleType>(*em1);
        TriangleEdgeScalarData<DoubleType> eder2 =
            TriangleEdgeScalarData<DoubleType>(*em2);

        eder0.times_equal_model(*ec); /// integrate wrt volume
        eder1.times_equal_model(*ec); /// integrate wrt volume
        eder2.times_equal_model(*ec); /// integrate wrt volume

        UnSymmetricTriangleEdgeAssembleJacobian(m, eder0, eder1, eder2, var,
                                                n0_sign, n1_sign);
      }
    }
  } else if (w == dsMathEnum::WhatToLoad::RHS) {
...
template <typename DoubleType>
void Equation<DoubleType>::TetrahedronEdgeCoupleAssemble(
    const std::string &model, dsMath::RealRowColValueVec<DoubleType> &m,
    dsMath::RHSEntryVec<DoubleType> &v, dsMathEnum::WhatToLoad w,
    const std::string &edge_couple, const DoubleType n0_sign,
    const DoubleType n1_sign) {
  ...
  if ((w == dsMathEnum::WhatToLoad::RHS) ||
      (w == dsMathEnum::WhatToLoad::MATRIXANDRHS)) {
    TetrahedronEdgeScalarData<DoubleType> eflux =
        TetrahedronEdgeScalarData<DoubleType>(*ef);
    eflux.times_equal_model(*ec);
    ...
    for (; it != vlist.end(); ++it) {
      ...
      if ((!em0) && (!em1) && (!em2) && (!em3)) {
        ...
        dsErrors::MissingEquationModel(r, myname, dermodel2,
                                       dsErrors::ModelInfo::ELEMENTEDGE,
                                       OutputStream::OutputType::VERBOSE1);
        dsErrors::MissingEquationModel(r, myname, dermodel3,
                                       dsErrors::ModelInfo::ELEMENTEDGE,
                                       OutputStream::OutputType::VERBOSE1);
      } else {
        if (!em0) {
        ...
        TetrahedronEdgeScalarData<DoubleType> eder0 =
            TetrahedronEdgeScalarData<DoubleType>(*em0);
        TetrahedronEdgeScalarData<DoubleType> eder1 =
            TetrahedronEdgeScalarData<DoubleType>(*em1);
        TetrahedronEdgeScalarData<DoubleType> eder2 =
            TetrahedronEdgeScalarData<DoubleType>(*em2);
        TetrahedronEdgeScalarData<DoubleType> eder3 =
            TetrahedronEdgeScalarData<DoubleType>(*em3);
        ...
        UnSymmetricTetrahedronEdgeAssembleJacobian(
            m, eder0, eder1, eder2, eder3, var, n0_sign, n1_sign);
      }
    }
  } else if (w == dsMathEnum::WhatToLoad::RHS) {
  } else {
...
template <typename DoubleType>
void Equation<DoubleType>::ElementEdgeCoupleAssemble(
    const std::string &model, dsMath::RealRowColValueVec<DoubleType> &m,
    dsMath::RHSEntryVec<DoubleType> &v, dsMathEnum::WhatToLoad w) {
...
template <typename DoubleType>
void Equation<DoubleType>::ElementNodeVolumeAssemble(
    const std::string &volume_node0_model,
    const std::string &volume_node1_model,
    dsMath::RealRowColValueVec<DoubleType> &m,
    dsMath::RHSEntryVec<DoubleType> &v, dsMathEnum::WhatToLoad w) {
  const size_t dimension = GetRegion().GetDimension();
  ...
  if (dimension == 2) {
    if ((node0model == node1model) &&
        (volume_node0_model == volume_node1_model)) {
      TriangleEdgeCoupleAssemble(volume_node0_model, m, v, w, node0model, 1.0,
                                 1.0);
    } else {
      /// necessary for cylindrical coordinate system in 2d
      TriangleEdgeCoupleAssemble(volume_node0_model, m, v, w, node0model, 1.0,
                                 0.0);
      TriangleEdgeCoupleAssemble(volume_node1_model, m, v, w, node1model, 0.0,
                                 1.0);
    }
  } else if (dimension == 3) {
    if ((node0model == node1model) &&
        (volume_node0_model == volume_node1_model)) {
      TetrahedronEdgeCoupleAssemble(volume_node0_model, m, v, w, node0model,
                                    1.0, 1.0);
    } else {
      TetrahedronEdgeCoupleAssemble(volume_node0_model, m, v, w, node0model,
                                    1.0, 0.0);
      TetrahedronEdgeCoupleAssemble(volume_node1_model, m, v, w, node1model,
                                    0.0, 1.0);
    }
  }
}
...
template <typename DoubleType>
std::string
Equation<DoubleType>::GetDerivativeModelName(const std::string &model,
                                             const std::string &var) {
...
template <typename DoubleType>
void Equation<DoubleType>::GetCommandOptions(
    std::map<std::string, ObjectHolder> &omap) const {
