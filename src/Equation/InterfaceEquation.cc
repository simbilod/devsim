...
InterfaceEquation<DoubleType>::InterfaceEquation(const std::string &nm,
                                                 const std::string &nm0,
...
namespace {
//
// if a contact has the same equation mapping, and shares a coincident node with
// this interface, it is removed
//
ConstNodeList_t RemoveContactNodesFromList(const Region &region,
                                           const ConstNodeList_t &inodes,
  ...
  // check all interface nodes for presence of a contact with the same equation
  // at the node
  for (auto &node : inodes) {
    ...
    // find all contacts at this coordinate and check to see if they solve this
    // equation
...
//
// If two interfaces with the same equation mapping share a coincident node,
// that node is removed
//
ConstNodeList_t RemoveInterfaceNodesFromList(const Interface &interface,
                                             const Region &region,
                                             const ConstNodeList_t &inodes,
                                             const std::string &eqname) {
  ConstNodeList_t ret;
  ...
  for (auto &node : inodes) {
    ...
    // I think this test is impossible, since this interface should be on the
    // coordinate
    if (iit == cti.end()) {
      ret.push_back(node);
      continue;
...
///// will skip this node
///// If an interface for this equation on this region already exists, we skip
///out on that
template <typename DoubleType>
ConstNodeList_t InterfaceEquation<DoubleType>::GetActiveNodesFromList(
    const Region &region, const ConstNodeList_t &inputnodes) const {
  ...
  const auto &ret =
      RemoveInterfaceNodesFromList(interface, region, cnodes, eqname);
...
std::set<ConstNodePtr> InterfaceEquation<DoubleType>::GetActiveNodes0() const {
  ...
  const auto &onodes0 =
      GetActiveNodesFromList(*GetInterface().GetRegion0(), inodes0);
...
std::set<ConstNodePtr> InterfaceEquation<DoubleType>::GetActiveNodes1() const {
  ...
  const auto &onodes1 =
      GetActiveNodesFromList(*GetInterface().GetRegion1(), inodes1);
...
template <typename DoubleType>
void InterfaceEquation<DoubleType>::Assemble(
...
namespace {
struct vlistdata {
  vlistdata(const std::string &m, const std::string &v, const Region &r,
            const ConstNodeList &nl)
      : interfacemodelname(m), var(v), region(&r), nodelist(&nl) {}
...
template <typename DoubleType>
void InterfaceEquation<DoubleType>::NodeVolumeType1Assemble(
    const std::string &interfacenodemodel,
    dsMath::RealRowColValueVec<DoubleType> &m,
    dsMath::RHSEntryVec<DoubleType> &v, PermutationMap &p,
    dsMathEnum::WhatToLoad w, const std::string & /*surface_area*/) {
...

  {
    ...
    if (eqindex0 == size_t(-1)) {
      os << "Could not find equation " << myname0 << " on region0 "
         << r0.GetName() << " for interface equation " << myname << "\n";
    }

    if (eqindex1 == size_t(-1)) {
      os << "Could not find equation " << myname1 << " on region1 "
         << r1.GetName() << " for interface equation " << myname << "\n";
    }
...

  {
    ...
    if (!inm.get()) {
      os << "Interface node model " << interfacenodemodel
         << " is not available\n";
    }
  ...
  if (w == dsMathEnum::WhatToLoad::PERMUTATIONSONLY) {
    ...
    for (size_t i = 0; i < vlist0.size(); ++i) {
      ...
      if (!idm) {
        dsErrors::MissingInterfaceEquationModel(
            r0, *this, dermodel, OutputStream::OutputType::VERBOSE1);
      } else {
    ...
    for (size_t i = 0; i < vlist1.size(); ++i) {
      ...
      if (!idm) {
        dsErrors::MissingInterfaceEquationModel(
            r1, *this, dermodel, OutputStream::OutputType::VERBOSE1);
      } else {
    ...
    for (size_t j = 0; j < vlistd.size(); ++j) {
      ...
      const size_t eqindex =
          reg.GetEquationIndex(reg.GetEquationNameFromVariable(var));
      dsAssert(eqindex != size_t(-1), "UNEXPECTED");

      ConstInterfaceNodeModelPtr idm = in.GetInterfaceNodeModel(imname);
      dsAssert(idm.get(),
               "UNEXPECTED"); // Existence is checked when added to vlistd

      const NodeScalarList<DoubleType> &vals =
          idm->GetScalarValues<DoubleType>();
...
template <typename DoubleType>
void InterfaceEquation<DoubleType>::NodeVolumeType2Assemble(
    const std::string &interfacenodemodel,
    dsMath::RealRowColValueVec<DoubleType> &m,
    dsMath::RHSEntryVec<DoubleType> &v, PermutationMap &p,
    dsMathEnum::WhatToLoad w, const std::string &surface_area) {
  ...
  /// It doesn't make sense if the equation doesn't exist in both regions of
  /// interface
  ...
  if ((w == dsMathEnum::WhatToLoad::RHS) ||
      (w == dsMathEnum::WhatToLoad::MATRIXANDRHS)) {
    const NodeScalarList<DoubleType> &sa0vals =
        sa0->GetScalarValues<DoubleType>();
    const NodeScalarList<DoubleType> &sa1vals =
        sa1->GetScalarValues<DoubleType>();
...
template <typename DoubleType>
void InterfaceEquation<DoubleType>::NodeVolumeType3Assemble(
    const std::string &interfacenodemodel,
    dsMath::RealRowColValueVec<DoubleType> &m,
    dsMath::RHSEntryVec<DoubleType> &v, PermutationMap &p,
    dsMathEnum::WhatToLoad w, const std::string &surface_area) {
    ...
    for (size_t j = 0; j < vlistd.size(); ++j) {
      ...
      for (size_t i = 0; i < nlist.size(); ++i) {
        ...
        m.push_back(dsMath::RealRowColVal<DoubleType>(row1, col, +val1));
             ...
             w == dsMathEnum::WhatToLoad::PERMUTATIONSONLY) {
  } else {
    dsAssert(0, "UNEXPECTED");
  }
}
...
template <typename DoubleType>
void InterfaceEquation<DoubleType>::GetCommandOptions(
