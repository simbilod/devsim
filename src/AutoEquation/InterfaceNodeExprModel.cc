...
InterfaceNodeExprModel<DoubleType>::InterfaceNodeExprModel(
    const std::string &nm, const Eqo::EqObjPtr eq, InterfacePtr ip)
    : InterfaceNodeModel(nm, ip), equation(eq) {
#if 0
...
void InterfaceNodeExprModel<DoubleType>::calcNodeScalarValues() const {
  ...
  if (!errors.empty()) {
    ...
    os << "while evaluating interface model " << GetName() << " on device "
       << GetInterface().GetRegion0()->GetDeviceName() << " on interface "
       << GetInterface().GetName() << " with expression "
       << EngineAPI::getStringValue(equation) << "\n";
    for (typename IMEE::InterfaceModelExprEval<DoubleType>::error_t::iterator
             it = errors.begin();
         it != errors.end(); ++it) {
      os << *it << "\n";
    }
    output_errors = os.str();
    GeometryStream::WriteOut(OutputStream::OutputType::ERROR, *ip,
                             output_errors);
  }

  if ((out.GetType() == IMEE::datatype::NODEDATA)) {
    ...
    os << "while evaluating model " << GetName() << ": expression "
       << EngineAPI::getStringValue(equation) << " evaluates to a "
       << IMEE::datatypename[static_cast<size_t>(out.GetType())] << "\n";
...
void InterfaceNodeExprModel<DoubleType>::Serialize(std::ostream &of) const {
  of << "COMMAND interface_model -device \"" << GetDeviceName()
     << "\" -interface \"" << GetInterfaceName() << "\" -name \"" << GetName()
     << "\" -equation \"" << EngineAPI::getStringValue(equation) << ";\"";
}
...
InterfaceNodeModelPtr CreateInterfaceNodeExprModel(const std::string &nm,
                                                   Eqo::EqObjPtr eq,
                                                   InterfacePtr ip) {
  const bool use_extended = ip->UseExtendedPrecisionModels();
  return create_interface_node_model<InterfaceNodeExprModel<double>,
                                     InterfaceNodeExprModel<extended_type>>(
      use_extended, nm, eq, ip);
}
