...
ModelExprData<DoubleType>::GetScalarValues() const {
...
ModelExprData<DoubleType>::operator=(const ModelExprData<DoubleType> &x) {
...
ModelExprData<DoubleType>::ModelExprData(const ModelExprData &x)
    : nodeScalarData(x.nodeScalarData), edgeScalarData(x.edgeScalarData),
      triangleEdgeScalarData(x.triangleEdgeScalarData),
      tetrahedronEdgeScalarData(x.tetrahedronEdgeScalarData), val(x.val),
      type(x.type), reg(x.reg) {}
...
template <typename DoubleType>
void convertEdgeToTetrahedronEdgeData(
    const Region &region, const std::vector<DoubleType> &edgeData,
...
template <typename T>
ModelExprData<DoubleType> &
ModelExprData<DoubleType>::op_equal(const ModelExprData<DoubleType> &other,
                                    const T &func) {
  switch (type) {
...
template <typename DoubleType>
ModelExprData<DoubleType> &
ModelExprData<DoubleType>::operator+=(const ModelExprData<DoubleType> &other) {
