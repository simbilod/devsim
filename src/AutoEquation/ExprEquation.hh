...
public:
  ExprEquation(
      const std::string &, /*Equation Name we are giving ourself*/
      ...
      const std::string
          &, /*ElementVolumeModel Name for integrating source (node 0)*/
      const std::string
          &, /*ElementVolumeModel Name for integrating source (node 1)*/
      const std::string
          &, /*Time-dependent NodeModel Name for integrating source/sink*/
...
private:
  ...
  void ACUpdateValues(NodeModel &,
                      const dsMath::ComplexDoubleVec_t<DoubleType> &);
  void NoiseUpdateValues(const std::string &,
