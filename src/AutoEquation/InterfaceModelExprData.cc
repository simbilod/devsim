...
const char *const datatypename[] = {"nodedata", "double", "invalid"};

/// Undefined behavior if we have a double or invalid data
template <typename DoubleType>
ScalarValuesType<DoubleType>
InterfaceModelExprData<DoubleType>::GetScalarValues() const {
  ScalarValuesType<DoubleType> ret;

  if (type == datatype::NODEDATA) {
    if (nsd->IsUniform()) {
      ret = ScalarValuesType<DoubleType>(nsd->GetUniformValue(),
                                         nsd->GetLength());
    } else {
...
template <typename DoubleType>
InterfaceModelExprData<DoubleType>::InterfaceModelExprData(
    const InterfaceModelExprData<DoubleType> &x)
    : nsd(x.nsd), val(x.val), type(x.type) {}

template <typename DoubleType>
InterfaceModelExprData<DoubleType>::InterfaceModelExprData(
    const InterfaceNodeScalarData<DoubleType> &x)
    : val(0.0), type(datatype::NODEDATA) {
  InterfaceNodeScalarData<DoubleType> *foo =
      new InterfaceNodeScalarData<DoubleType>(x);
  nsd = std::shared_ptr<InterfaceNodeScalarData<DoubleType>>(foo);
};

template <typename DoubleType>
InterfaceModelExprData<DoubleType> &
InterfaceModelExprData<DoubleType>::operator=(
    const InterfaceModelExprData<DoubleType> &x) {
...
template <typename T>
InterfaceModelExprData<DoubleType> &
InterfaceModelExprData<DoubleType>::op_equal(
    const InterfaceModelExprData<DoubleType> &other, const T &func) {
  makeUnique();
  ...
  if (type == datatype::NODEDATA) {
    ...
    if (other.type == datatype::NODEDATA) {
      InterfaceNodeScalarData<DoubleType> *x =
          new InterfaceNodeScalarData<DoubleType>(val, other.nsd->GetLength());
      nsd = nsd_ptr(x);
      nsd->op_equal_data(*other.nsd, func);
...
InterfaceModelExprData<DoubleType> &
InterfaceModelExprData<DoubleType>::operator*=(
    const InterfaceModelExprData<DoubleType> &other) {
