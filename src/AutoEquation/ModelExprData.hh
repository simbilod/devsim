...
namespace MEE {
template <typename DoubleType> class ScalarValuesType {
public:
  ScalarValuesType() : vals(0), uval(0.0), isuniform(false), length(0) {}
  ScalarValuesType(const std::vector<DoubleType> &v)
      : vals(&v), uval(0.0), isuniform(false) {
    length = v.size();
  }

  ScalarValuesType(DoubleType v, size_t len)
      : vals(0), uval(v), isuniform(true), length(len) {}

  bool IsUniform() const { return isuniform; }

  const std::vector<DoubleType> &GetVector() const { return *vals; }

  DoubleType GetScalar() const { return uval; }
...
template <typename T>
using tetrahedronEdgeScalarData_ptr =
    std::shared_ptr<TetrahedronEdgeScalarData<T>>;

template <typename DoubleType> class ModelExprData {
public:
  explicit ModelExprData(const Region *r = nullptr)
      : val(0.0), type(datatype::INVALID), reg(r){};
  ...
  template <typename T>
  ModelExprData<DoubleType> &op_equal(const ModelExprData<DoubleType> &,
                                      const T &);
  ModelExprData<DoubleType> &operator*=(const ModelExprData &);
  ModelExprData<DoubleType> &operator+=(const ModelExprData &);

  // test or will get undefined data
  DoubleType GetDoubleValue() const { return val; }
...
private:
  template <typename T> void node_op_equal(const ModelExprData &, const T &);
  template <typename T> void edge_op_equal(const ModelExprData &, const T &);
  template <typename T>
  void triangle_edge_op_equal(const ModelExprData &, const T &);
  template <typename T>
  void tetrahedron_edge_op_equal(const ModelExprData &, const T &);
  template <typename T> void double_op_equal(const ModelExprData &, const T &);
  ...
  */
  ...
  //      void morph();
  typedef ModelExprData<DoubleType> &(ModelExprData<DoubleType>::*selfopptr)(
