...
public:
  ...
  ExprContactEquation(const std::string &eq, // equation
                      ContactPtr, RegionPtr,
...
private:
  ...
  void DerivedAssemble(dsMath::RealRowColValueVec<DoubleType> &,
  ...
  /// If both of the following strings are empty, we do not permute out the
  /// original ones.  Note these models have been calculated over the whole
  /// bulk.
  ...
  //// For the time being assume that we don't have an internal time
  ///displacement term / This may be important for different boundary
  ///conditions.
  const std::string
      nodemodel_current; /// model being used for external net flux
  const std::string
      edgemodel_current; /// model being used for external net flux
  const std::string
      elementedgemodel_current; /// model being used for external net flux
  const std::string
      nodemodel_charge; /// model being used for external net charge
  const std::string
      edgemodel_charge; /// model being used for external net charge
  const std::string
      elementedgemodel_charge; /// model being used for external net charge
  /// We assume, that the original equations will be coupled out and used for
  /// fully coupled simulation. In addition, they will be used for calculating
