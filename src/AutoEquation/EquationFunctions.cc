...
public:
  ...
  bool inModelList(const std::string &x) {
    bool inlist = false;
    if (region_) {
      inlist = (inNodeModelList(x, region_) || inEdgeModelList(x, region_) ||
...
bool neverInModelList(const std::string &) { return false; }
...
Eqo::EqObjPtr DefaultDevsimDerivative(Eqo::EqObjPtr self, Eqo::EqObjPtr foo) {
  ...
  if (oval == value) {
    ...
    //// TODO: Set way of specifying whether our expression is on an interface
    ///or a device / TODO: Fix this hack
...
Eqo::EqObjPtr CreateExprModel(const std::string &nm, const std::string &expr,
                              RegionPtr rp, std::string &errorstring) {
...
/// Differs only really in the model list and pointer to data (interface versus
/// region)
Eqo::EqObjPtr CreateInterfaceExprModel(const std::string &nm,
                                       const std::string &expr, InterfacePtr ip,
...
getTriangleEdgeModelDisplayType(const std::string &dt) {
  TriangleEdgeModel::DisplayType ret =
...
getTetrahedronEdgeModelDisplayType(const std::string &dt) {
  TetrahedronEdgeModel::DisplayType ret =
...
ret_pair CreateNodeExprModel(const std::string &nm, const std::string &expr,
                             RegionPtr rp, NodeModel::DisplayType dt,
                             ContactPtr cp) {
  bool ret = false;
  std::string error;
  ...
  if (error.empty()) {
    ...
    /// Create helper function to worry about this, which will be called before
    /// this
...
ret_pair CreateEdgeExprModel(const std::string &nm, const std::string &expr,
                             RegionPtr rp, EdgeModel::DisplayType dt,
                             ContactPtr cp) {
  bool ret = false;
  std::string error;
  Eqo::EqObjPtr equation;
...
ret_pair CreateTetrahedronEdgeExprModel(const std::string &nm,
                                        const std::string &expr, RegionPtr rp,
