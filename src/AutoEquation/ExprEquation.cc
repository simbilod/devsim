...
ExprEquation<DoubleType>::ExprEquation(
    const std::string &eqname, RegionPtr rp, const std::string &var,
    const std::string &nmodel, const std::string &emodel,
    const std::string &evmodel, const std::string &eemodel,
    const std::string &eev0model, const std::string &eev1model,
    const std::string &tdnmodel, EquationEnum::UpdateType ut)
    : Equation<DoubleType>(eqname, rp, var, ut), node_model_(nmodel),
      edge_model_(emodel), edge_volume_model_(evmodel), element_model_(eemodel),
      volume_node0_model_(eev0model), volume_node1_model_(eev1model),
      time_node_model_(tdnmodel) {}

template <typename DoubleType>
void ExprEquation<DoubleType>::DerivedAssemble(
  ...
  if (t == dsMathEnum::TimeMode::DC) {
    if (!edge_model_.empty()) {
      ...
      if (!edge_volume_model_.empty()) {
        Equation<DoubleType>::EdgeNodeVolumeAssemble(edge_volume_model_, m, v,
                                                     w);
      }
    }
    ...
    if (!volume_node0_model_.empty() || !volume_node1_model_.empty()) {
      model_cache->clear();
      Equation<DoubleType>::ElementNodeVolumeAssemble(
          volume_node0_model_, volume_node1_model_, m, v, w);
    }
...
template <typename DoubleType>
void ExprEquation<DoubleType>::UpdateValues(
    NodeModel &nm, const dsMath::DoubleVec_t<DoubleType> &rhs) {
  Equation<DoubleType>::DefaultUpdate(nm, rhs);
}

template <typename DoubleType>
void ExprEquation<DoubleType>::ACUpdateValues(
    NodeModel &nm, const dsMath::ComplexDoubleVec_t<DoubleType> &rhs) {
  Equation<DoubleType>::DefaultACUpdate(nm, rhs);
}

template <typename DoubleType>
void ExprEquation<DoubleType>::NoiseUpdateValues(
...
void ExprEquation<DoubleType>::Serialize(std::ostream &of) const {
  of << "COMMAND equation -device \""
     << Equation<DoubleType>::GetRegion().GetDeviceName() << "\" -region \""
     << Equation<DoubleType>::GetRegion().GetName() << "\" -name \""
     << Equation<DoubleType>::GetName() << "\" -variable_name \""
     << Equation<DoubleType>::GetVariable() << "\" -node_model \""
     << node_model_ << "\" -edge_model \"" << edge_model_
     << "\" -edge_volume_model \"" << edge_volume_model_
     << "\" -element_model \"" << element_model_ << "\" -volume_node0_model \""
     << volume_node0_model_ << "\" -volume_node1_model \""
     << volume_node1_model_ << "\" -time_node_model \"" << time_node_model_
     << "\"";
}

template <typename DoubleType>
void ExprEquation<DoubleType>::GetCommandOptions_Impl(
    std::map<std::string, ObjectHolder> &omap) const {
  omap["device"] =
      ObjectHolder(Equation<DoubleType>::GetRegion().GetDeviceName());
