...
NodeExprModel<DoubleType>::NodeExprModel(const std::string &nm,
                                         const Eqo::EqObjPtr eq, RegionPtr rp,
                                         NodeModel::DisplayType dt,
                                         ContactPtr cp)
    : NodeModel(nm, rp, dt, cp), equation(eq) {
#if 0
...
//    dsAssert(false, "UNEXPECTED");
  ...
  if ((out.GetType() == MEE::datatype::NODEDATA)) {
...
void NodeExprModel<DoubleType>::Serialize(std::ostream &of) const {
  ...
  if (AtContact()) {
    of << "contact_node_model -device \"" << GetDeviceName() << "\" -contact \""
       << GetContactName() << "\" ";
  } else {
    of << "node_model -device \"" << GetDeviceName() << "\" -region \""
       << GetRegionName() << "\" ";
  }
  ...
  of << "-display_type \"" << GetDisplayTypeString()
     << "\" "
        "-name \""
     << GetName() << "\" -equation \"" << EngineAPI::getStringValue(equation)
     << ";\"";
}
...
NodeModelPtr CreateNodeExprModel(const std::string &nm, const Eqo::EqObjPtr eq,
                                 RegionPtr rp, NodeModel::DisplayType dt,
                                 ContactPtr cp) {
  const bool use_extended = rp->UseExtendedPrecisionModels();
  return create_node_model<NodeExprModel<double>, NodeExprModel<extended_type>>(
      use_extended, nm, eq, rp, dt, cp);
}
