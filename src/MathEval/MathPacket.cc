...
template <typename DoubleType>
MathPacket<DoubleType>::MathPacket(
    const MathWrapper<DoubleType> &wrapper,
    const std::vector<DoubleType> &dvals,
    const std::vector<const std::vector<DoubleType> *> &vvals,
    std::vector<DoubleType> &result)
    : wrapperClass_(wrapper), dvals_(dvals), vvals_(vvals), result_(result),
      fpeFlag_(FPECheck::getClearedFlag()), num_processed_(0) {}
...
void MathPacket<DoubleType>::operator()(size_t vbeg, size_t vend) {
  ...
  ///// This can be asserted since we already know the main thread id on
  ///application startup
  FPECheck::ClearFPE();
...
template <typename DoubleType>
std::string
MathPacketRun(const MathWrapper<DoubleType> &func,
              const std::vector<DoubleType> &dvals,
              const std::vector<const std::vector<DoubleType> *> &vvals,
              std::vector<DoubleType> &result, size_t vlen) {
  ...
  if ((num_threads > 1) && vlen > task_size) {
    ...
    while (b < e) {
      auto packet = std::shared_ptr<Eqomfp::MathPacket<DoubleType>>(
          new MathPacket<DoubleType>(func, dvals, vvals, result));
      packets.push_back(packet);
      futures.push_back(std::async(std::launch::async,
                                   MathPacketRange<DoubleType>(*packet, b, e)));

      b = e;
      e += step;
      //// If we have 2 threads, take odd off so don't need additional queue
      ///member / or just use a magic number
      if (e > (vlen - 2)) {
        e = vlen;
      }
    }
...
template <typename DoubleType>
MathPacketRange<DoubleType>::MathPacketRange(MathPacket<DoubleType> &mp,
                                             size_t b, size_t e)
    : mathPacket_(mp), beg_(b), end_(e) {}

template <typename DoubleType> void MathPacketRange<DoubleType>::operator()() {
  mathPacket_(beg_, end_);
}

template class MathPacket<double>;
template std::string
MathPacketRun(const MathWrapper<double> &, const std::vector<double> &,
              const std::vector<const std::vector<double> *> &,
              std::vector<double> &, size_t);
#ifdef DEVSIM_EXTENDED_PRECISION
template class MathPacket<float128>;
template std::string
MathPacketRun(const MathWrapper<float128> &, const std::vector<float128> &,
              const std::vector<const std::vector<float128> *> &,
              std::vector<float128> &, size_t);
#endif
} // namespace Eqomfp
