...
namespace Eqomfp {
template <typename T> struct TernaryTblEntry {
  const char *name;
  ternaryfuncptr<T> func;
  const char *desc;
};

template <typename T> struct QuaternaryTblEntry {
  const char *name;
  quaternaryfuncptr<T> func;
  const char *desc;
};

template <typename T> struct BinaryTblEntry {
  const char *name;
  binaryfuncptr<T> func;
  const char *desc;
};

template <typename T> struct UnaryTblEntry {
  const char *name;
  unaryfuncptr<T> func;
  const char *desc;
};
...
template <> Eqomfp::UnaryTblEntry<double> &Tables::GetUnaryTable(size_t i) {
  return UnaryTable_double[i];
}
template <> Eqomfp::BinaryTblEntry<double> &Tables::GetBinaryTable(size_t i) {
  return BinaryTable_double[i];
}
template <> Eqomfp::TernaryTblEntry<double> &Tables::GetTernaryTable(size_t i) {
...
#ifdef DEVSIM_EXTENDED_PRECISION
template <> Eqomfp::UnaryTblEntry<float128> &Tables::GetUnaryTable(size_t i) {
  return UnaryTable_float128[i];
}
template <> Eqomfp::BinaryTblEntry<float128> &Tables::GetBinaryTable(size_t i) {
...
template <typename DoubleType> MathEval<DoubleType>::MathEval() {}
...
template <typename DoubleType> void MathEval<DoubleType>::DestroyInstance() {
  delete instance_;
  instance_ = nullptr;
}
