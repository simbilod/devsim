...
template <typename T> T Joyce(T r) {
...
template <typename T> T dJoycedx(T r) {
...
template <typename T> T Expansion(T r) {
  /// M_PI_4 = pi/4
...
template <typename T> T dExpansiondx(T r) {
...
template <typename T> T InvFermi(T r) {
  if (r < breakpoint) {
    return Joyce(r);
  } else {
    return Expansion(r);
...
template <typename T> T dInvFermidx(T r) {
  if (r < breakpoint) {
    return dJoycedx(r);
  } else {
...
//#include <iostream>
template <typename T> T Fermi(T Eta) {
  //// initial guess
  T r = exp(Eta);
...
///// This is inefficient so be sure to program special node model depending on
///ferm result
template <typename T> T dFermidx(T Eta) {
...
#include <iomanip>
#include <iostream>
template <typename T> T pdiff(T x1, T x2) {
  return 2.0 * (x1 - x2) / (x1 + x2);
}
template <typename T> void unit() {
  std::cout << std::setprecision(std::numeric_limits<T>::max_digits10);
  for (T x = 8.46296036621705; x < 8.46296036621707; x += 0.0000001) {
    std::cout << x << " " << InvFermi(x) << " " << Joyce(x) << " "
              << Expansion(x) << " " << dJoycedx(x) << " " << dExpansiondx(x)
              << " " << pdiff(dJoycedx(x), dExpansiondx(x)) << " "
              << Expansion(x) - Joyce(x) << "\n";
  }
