...
namespace dsCommand {
void createEquationCmd(CommandHandler &data) {
  ...
  static dsGetArgs::Option option[] = {
      ...
      {"variable_name", "", dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::REQUIRED, stringCannotBeEmpty},
      {"node_model", "", dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::OPTIONAL, nullptr},
      {"edge_model", "", dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::OPTIONAL, nullptr},
      {"edge_volume_model", "", dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::OPTIONAL, nullptr},
      {"element_model", "", dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::OPTIONAL, nullptr},
      {"volume_node0_model", "", dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::OPTIONAL, nullptr},
      {"volume_node1_model", "", dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::OPTIONAL, nullptr},
      {"time_node_model", "", dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::OPTIONAL, nullptr},
      {"variable_update", "default", dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::OPTIONAL, nullptr},
  ...
  const std::string &edge_volume_model =
      data.GetStringOption("edge_volume_model");
  const std::string &elementedge_model = data.GetStringOption("element_model");
  const std::string &volume_node0_model =
      data.GetStringOption("volume_node0_model");
  const std::string &volume_node1_model =
      data.GetStringOption("volume_node1_model");
  ...
  if (reg->UseExtendedPrecisionEquations()) {
    new ExprEquation<extended_type>(
        name, reg, variable_name, node_model, edge_model, edge_volume_model,
        elementedge_model, volume_node0_model, volume_node1_model,
        time_node_model, updateType);
  } else {
    new ExprEquation<double>(name, reg, variable_name, node_model, edge_model,
                             edge_volume_model, elementedge_model,
...
void getEquationListCmd(CommandHandler &data) {
  std::string errorString;
...
void deleteEquationCmd(CommandHandler &data) {
  std::string errorString;
...
void createInterfaceEquationCmd(CommandHandler &data) {
  std::string errorString;
  ...
  static dsGetArgs::Option option[] = {
      ...
      {"type", "", dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::REQUIRED, stringCannotBeEmpty},
      {"interface_model", "", dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::OPTIONAL, nullptr},
      {"name0", "", dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::OPTIONAL, nullptr},
      {"name1", "", dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::OPTIONAL, nullptr},
      {nullptr, nullptr, dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::OPTIONAL, nullptr}};
  ...
  // data.GetStringOption("variable_name");
  const std::string &deviceName = data.GetStringOption("device");
  ...
  InterfaceExprEquationEnum::EquationType et =
      InterfaceExprEquationEnum::UNKNOWN;
  if (type == "continuous") {
    ...
    std::ostringstream os;
    os << "-type must be specified as \"continuous\", \"fluxterm\", or "
          "\"hybrid\"\n";
    errorString += os.str();
  }
  ...
  if (interface->UseExtendedPrecisionEquations()) {
    new InterfaceExprEquation<extended_type>(name, name0, name1, interface,
                                             interface_model, et);
  } else {
    new InterfaceExprEquation<double>(name, name0, name1, interface,
                                      interface_model, et);
  }
  data.SetEmptyResult();
}

void getInterfaceEquationListCmd(CommandHandler &data) {
  std::string errorString;
...
void deleteInterfaceEquationCmd(CommandHandler &data) {
  std::string errorString;
...
void createContactEquationCmd(CommandHandler &data) {
  std::string errorString;
  ...
  static dsGetArgs::Option option[] = {
      ...
      {"volume_node1_model", "", dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::OPTIONAL},
      {"node_current_model", "", dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::OPTIONAL},
      {"edge_current_model", "", dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::OPTIONAL},
      {"element_current_model", "", dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::OPTIONAL},
      {"node_charge_model", "", dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::OPTIONAL},
      {"edge_charge_model", "", dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::OPTIONAL},
      {"element_charge_model", "", dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::OPTIONAL},
      {"circuit_node", "", dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::OPTIONAL},
      //        {"variable_name", "", dsGetArgs::optionType::STRING,
      //        dsGetArgs::requiredType::OPTIONAL},
      {nullptr, nullptr, dsGetArgs::optionType::STRING,
  ...
  const std::string &volume_node1_model =
      data.GetStringOption("volume_node1_model");
  const std::string &node_charge_model =
      data.GetStringOption("node_charge_model");
  const std::string &edge_charge_model =
      data.GetStringOption("edge_charge_model");
  const std::string &element_charge_model =
      data.GetStringOption("element_charge_model");
  const std::string &node_current_model =
      data.GetStringOption("node_current_model");
  const std::string &edge_current_model =
      data.GetStringOption("edge_current_model");
  const std::string &element_current_model =
      data.GetStringOption("element_current_model");
  ...
  if (region->UseExtendedPrecisionEquations()) {
    ContactEquation<extended_type> *ce = new ExprContactEquation<extended_type>(
        name, contact, region, node_model, edge_model, edge_volume_model,
        element_model, volume_node0_model, volume_node1_model,
        node_current_model, edge_current_model, element_current_model,
        node_charge_model, edge_charge_model, element_charge_model);
    if (!circuit_node.empty()) {
      ce->SetCircuitNode(circuit_node);
    }
  } else {
    ContactEquation<double> *ce = new ExprContactEquation<double>(
        name, contact, region, node_model, edge_model, edge_volume_model,
        element_model, volume_node0_model, volume_node1_model,
        node_current_model, edge_current_model, element_current_model,
        node_charge_model, edge_charge_model, element_charge_model);
...
void getContactEquationListCmd(CommandHandler &data) {
  std::string errorString;
...
void deleteContactEquationCmd(CommandHandler &data) {
  std::string errorString;
...
void createCustomEquationCmd(CommandHandler &data) {
  std::string errorString;
...
void getEquationNumbersCmd(CommandHandler &data) {
  std::string errorString;
  ...
  static dsGetArgs::Option option[] = {
      ...
      {"region", "", dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::REQUIRED, mustBeValidRegion},
      {"variable", "", dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::OPTIONAL, nullptr},
  ...
  if (!equationName.empty()) {
    ...
    if (eqindex == size_t(-1)) {
      errorString += std::string("Could not find equation index for ") +
                     equationName + "\n";
    } else {
      ...
      for (ConstNodeList::const_iterator it = cnl.begin(); it != cnl.end();
           ++it) {
