...
std::string onInterfaceonDevice(const std::string &inm,
                                const std::string &dnm) {
  std::ostringstream os;
...
std::string ValidateDeviceAndRegion(const std::string &deviceName,
                                    const std::string &regionName, Device *&dev,
                                    Region *&reg) {
  dev = nullptr;
  reg = nullptr;
...
std::string ValidateDeviceRegionAndContact(const std::string &deviceName,
                                           ...
                                           const std::string &contactName,
                                           Device *&dev, Region *&reg,
                                           Contact *&contact) {
  contact = nullptr;
  std::string errorString =
      ValidateDeviceAndRegion(deviceName, regionName, dev, reg);
  ...
  if (!contact || (contact->GetRegion() != reg)) {
    std::ostringstream os;
    os << "Contact \"" << contactName << "\" does not exist "
       << onRegiononDevice(deviceName, regionName) << "\n";
    errorString = os.str();
  }
  return errorString;
}
...
std::string ValidateDeviceAndInterface(const std::string &deviceName,
  ...
  if (!interface) {
    std::ostringstream os;
    os << "Interface \"" << interfaceName << "\" on Device \"" << deviceName
       << "\" does not exist.";
    errorString = os.str();
  }
...
std::string ValidateDeviceRegionAndInterface(const std::string &deviceName,
  ...
  if (!interface) {
    ...
    os << "Interface \"" << interfaceName << "\" on Device \"" << deviceName
       << "\" does not exist.";
    errorString = os.str();
  } else if ((interface->GetRegion0() != region) &&
             (interface->GetRegion1() != region)) {
    std::ostringstream os;
    os << "Region \"" << regionName << "\" on Device \"" << deviceName
       << "\" on interface \"" << interfaceName << "\" does not exist.";
    errorString = os.str();
  }
...
std::string ValidateNodeModelName(Device *const dev, Region *const reg,
                                  const std::string &node_model) {
  std::string errorString;
  std::ostringstream os;

  if (node_model.empty()) {
    os << "-node_model cannot be an empty string "
       << onRegiononDevice(reg->GetName(), dev->GetName()) << "\n";
    errorString = os.str();
  } else {
    ConstNodeModelPtr nm = reg->GetNodeModel(node_model);
    if (!nm) {
      os << "-node_model \"" << node_model << "\""
         << onRegiononDevice(reg->GetName(), dev->GetName())
         << " does not exist\n";
      errorString = os.str();
    }
...
std::string ValidateEdgeModelName(Device *const dev, Region *const reg,
                                  const std::string &edge_model) {
  std::string errorString;
  std::ostringstream os;
  ...
  if (edge_model.empty()) {
    ...
    if (!nm) {
      os << "-edge_model \"" << edge_model << "\" on Region \""
         << reg->GetName() << "\" on Device \"" << dev->GetName()
         << "\" does not exist\n";
      errorString = os.str();
    }
  }
...
std::string ValidateOptionalNodeModelName(Device *const dev, Region *const reg,
                                          const std::string &node_model) {
  std::string errorString;
  if (!node_model.empty()) {
...
std::string ValidateOptionalEdgeModelName(Device *const dev, Region *const reg,
                                          const std::string &node_model) {
  std::string errorString;
  if (!node_model.empty()) {
...
std::string ValidateInterfaceNodeModelName(Device *const dev,
                                           Interface *const interface,
                                           const std::string &interface_model) {
  std::string errorString;
  std::ostringstream os;

  if (interface_model.empty()) {
    os << "-interface_model cannot be an empty string "
       << onInterfaceonDevice(interface->GetName(), dev->GetName()) << "\n";
    errorString = os.str();
  } else {
    ConstInterfaceNodeModelPtr nm =
        interface->GetInterfaceNodeModel(interface_model);
    if (!nm) {
      os << "-interface_model \"" << interface_model << "\""
         << onInterfaceonDevice(interface->GetName(), dev->GetName())
         << " does not exist\n";
      errorString = os.str();
    }
