...
void addCircuitNodeCmd(CommandHandler &data) {
  ...
  if (nk.IsCircuitNode(name)) {
    std::ostringstream os;
    os << "circuit node \"" << name << "\"  already exists\n";
    errorString += os.str();
  } else {
...
void circuitElementCmd(CommandHandler &data) {
  std::string errorString;
  ...
  static dsGetArgs::Option option[] = {
      {"name", "", dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::REQUIRED, stringCannotBeEmpty},
      {"n1", "", dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::REQUIRED, nullptr},
      {"n2", "", dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::REQUIRED, nullptr},
      {"value", "0.0", dsGetArgs::optionType::FLOAT,
       dsGetArgs::requiredType::OPTIONAL},
      {"acreal", "0.0", dsGetArgs::optionType::FLOAT,
       dsGetArgs::requiredType::OPTIONAL},
      {"acimag", "0.0", dsGetArgs::optionType::FLOAT,
       dsGetArgs::requiredType::OPTIONAL},
      {nullptr, nullptr, dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::OPTIONAL}};
  ...
  if (type != 'V' && type != 'v') {
    if ((acreal != 0.0) || (acimag != 0.0)) {
      std::ostringstream os;
      os << "Cannot specify -acreal or -acimag for non-voltage circuit element "
            " \""
         << name << "\"\n";
      errorString += os.str();
      error = true;
  ...
  if (type == 'V' || type == 'v') {
    ...
    im = new IdealVoltage(&nk, name.c_str(), n1.c_str(), n2.c_str(), value,
                          acreal, acimag, x);
    ...
    std::ostringstream os;
    os << "Cannot figure out what element type -name  \"" << name << "\" is\n";
    errorString += os.str();
  }
...
void circuitAlterCmd(CommandHandler &data) {
  std::string errorString;
  ...
  static dsGetArgs::Option option[] = {
      {"name", "", dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::REQUIRED, stringCannotBeEmpty},
      {"param", "value", dsGetArgs::optionType::STRING,
  ...
  if (ok) {
    ...
    os << "Circuit element -name  \"" << name
       << "\" does not accept parameter \"" << param << "\" with value \""
       << value << "\"\n";
    errorString += os.str();
  }
...
void circuitNodeAliasCmd(CommandHandler &data) {
  AddGroundNode();
  ...
  /// (This would be on the contact and not the contact equation??)
  static dsGetArgs::Option option[] = {
      {"node", "", dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::REQUIRED, stringCannotBeEmpty},
      {"alias", "", dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::REQUIRED, stringCannotBeEmpty},
      {nullptr, nullptr, dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::OPTIONAL, nullptr}};
...
void circuitGetCircuitNodeListCmd(CommandHandler &data) {
  ...
  for (NodeKeeper::NodeTable_t::const_iterator it = nt.begin(); it != nt.end();
       ++it) {
...
void circuitGetCircuitNodeValueCmd(CommandHandler &data) {
  ...
  static dsGetArgs::Option option[] = {
      {"node", "", dsGetArgs::optionType::STRING,
       dsGetArgs::requiredType::REQUIRED, stringCannotBeEmpty},
      {"solution", "dcop", dsGetArgs::optionType::STRING,
...
void circuitDeleteCircuitCmd(CommandHandler &data) {
  std::string errorString;
...
void circuitGetCircuitEquationNumberCmd(CommandHandler &data) {
  std::string errorString;
