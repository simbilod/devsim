...
TriangleCylindricalNodeVolume<DoubleType>::TriangleCylindricalNodeVolume(
    RegionPtr rp)
    : TriangleEdgeModel("ElementCylindricalNodeVolume@en0", rp,
                        TriangleEdgeModel::DisplayType::SCALAR) {
  node1Volume_ = TriangleEdgeSubModel<DoubleType>::CreateTriangleEdgeSubModel(
      "ElementCylindricalNodeVolume@en1", rp,
      TriangleEdgeModel::DisplayType::SCALAR, this->GetSelfPtr());
  RegisterCallback("raxis_zero");
  RegisterCallback("raxis_variable");
}

template <typename DoubleType>
void TriangleCylindricalNodeVolume<DoubleType>::calcTriangleEdgeScalarValues()
    const {
  ...
  for (size_t i = 0; i < el.size(); ++i) {
    const std::vector<DoubleType> &v =
        calcTriangleCylindricalNodeVolume(el[i], RAxisVariable, RAxis0);
...
namespace {
// Calc volume of triangle with flat top or bottom (constant z) on at least one
// side assume r1 and r2 are at z1
template <typename DoubleType>
DoubleType calcCylindricalTriangleVolumeFlat(DoubleType r0, DoubleType z0,
                                             DoubleType r1, DoubleType z1,
                                             DoubleType r2) {
  DoubleType ret = 0.0;
...
template <typename DoubleType>
DoubleType calcCylindricalTriangleVolume(
    std::vector<std::pair<DoubleType, DoubleType>> &pairs) {
  DoubleType ret = 0.0;
  /// sort by height
  ...
  if (z0 == z1) {
    ...
    //// since we are sorted by increasing z, a good triangle should have a
    ///valid denominator
...
TriangleCylindricalNodeVolume<DoubleType>::calcTriangleCylindricalNodeVolume(
...
template <typename DoubleType>
void TriangleCylindricalNodeVolume<DoubleType>::Serialize(
    std::ostream &of) const {
  of << "COMMAND element_cylindrical_node_volume -device \"" << GetDeviceName()
...
TriangleEdgeModelPtr CreateTriangleCylindricalNodeVolume(RegionPtr rp) {
  ...
  return create_triangle_edge_model<
      TriangleCylindricalNodeVolume<double>,
      TriangleCylindricalNodeVolume<extended_type>>(use_extended, rp);
}
