...
static const unsigned char base64Chars[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
...
std::string encodeBase64(const char *input, size_t length) {
  ...
  if (remainder == 1) {
    ...
    triplet[0] = base64Chars[(ptr[triplet_length] >> 2) & 0x3F];
    triplet[1] = base64Chars[((ptr[triplet_length] << 4) |
                              (ptr[triplet_length + 1] >> 4)) &
                             0x3F];
...
template <typename T>
std::string convertVectorToBase64(const std::vector<T> &x) {
...
template <typename T>
std::string convertVectorToZlibBase64(const std::vector<T> &x) {
  ...
  for (size_t i = 0; i < numberOfBlocks; ++i) {
    ...
    Bytef *inputArray =
        reinterpret_cast<Bytef *>(const_cast<T *>(&x[numberOfTPerBlock * i]));

    uLong outputLength = maxLength;
    int zlibRet = compress2(&(outputArray[0]), &outputLength, inputArray,
                            inputLength, Z_DEFAULT_COMPRESSION);
    dsAssert(zlibRet == Z_OK, "UNEXPECTED");

    compressedOutput += std::string(
        reinterpret_cast<const char *>(&outputArray[0]), outputLength);
    header[3 + i] = outputLength;
  }
  const std::string encodedData =
      encodeBase64(compressedOutput.c_str(), compressedOutput.size());
