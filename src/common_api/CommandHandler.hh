...
typedef bool (*checkFunction)(const std::string & /*optionname*/,
...
std::vector<T> GetKeys(const std::map<T, U> &tmap) {
  std::vector<T> result;
  for (typename std::map<T, U>::const_iterator it = tmap.begin();
       it != tmap.end(); ++it) {
