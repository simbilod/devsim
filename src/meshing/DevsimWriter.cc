...
void WriteNodeModels(std::ostream &myfile,
  ...
  for (Region::NodeModelList_t::const_iterator nit = nmlist.begin();
    ...
    for (NodeScalarList<double>::const_iterator vit = vals.begin();
         vit != vals.end(); ++vit) {
      myfile << *vit << "\n";
    }
...
void WriteEdgeModels(std::ostream &myfile,
  ...
  for (Region::EdgeModelList_t::const_iterator nit = emlist.begin();
    ...
    for (EdgeScalarList<double>::const_iterator vit = vals.begin();
         vit != vals.end(); ++vit) {
      myfile << *vit << "\n";
    }
...
bool DevsimWriter::WriteMesh_(const std::string &deviceName,
...
bool DevsimWriter::WriteMeshes_(const std::string &filename,
                                std::string &errorString) {
  bool ret = true;
  std::ostringstream os;
