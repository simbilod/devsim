...
void WriteNodesAndSolutions(std::ostream &myfile, const Region &reg,
  ...
  for (size_t i = 0; i < nvals; ++i) {
    ...
    for (std::vector<ConstNodeModelPtr>::iterator nmit = nmodels.begin();
         nmit != nmodels.end(); ++nmit) {
      myfile << " ";
      if (*nmit) {
...
void WriteEdgesAndSolutions(std::ostream &myfile, const Region &reg,
                            const std::set<std::string> solutions) {
  ...
  for (size_t i = 0; i < nvals; ++i) {
    ...
    for (std::vector<ConstEdgeModelPtr>::iterator emit = emodels.begin();
         emit != emodels.end(); ++emit) {
      myfile << " ";
      if (*emit) {
...
bool WriteSingleDevice(const std::string &dname, std::ostream &myfile,
  ...
  if (!dp) {
    ...
    std::set<std::string> edgeModels;
    {
      ...
      myfile << "s";
      for (std::set<std::string>::iterator nmit = nodeModels.begin();
           nmit != nodeModels.end(); ++nmit) {
        myfile << " " << *nmit;
      }
      myfile << "\n";

      myfile << "se";
      for (std::set<std::string>::iterator emit = edgeModels.begin();
           emit != edgeModels.end(); ++emit) {
        myfile << " " << *emit;
      }
      myfile << "\n";
...
bool FloodsWriter::WriteMesh_(const std::string &deviceName,
...
bool FloodsWriter::WriteMeshes_(const std::string &filename,
                                std::string &errorString) {
  bool ret = true;
  std::ostringstream os;
