...
class BoundingBox {
public:
  BoundingBox(double x0, double x1, double y0, double y1, double bl)
      : x0_(x0), x1_(x1), y0_(y0), y1_(y1), bloat_(bl) {
    if (x0 > x1) {
      x1_ = x0;
      x0_ = x1;
    }
  ...
  bool IsPointInside(double x, double y) const {
    return ((x > (x0_ - bloat_)) && (x < (x1_ + bloat_)) &&
            (y > (y0_ - bloat_)) && (y < (y1_ + bloat_)));
  }
  ...
  double GetBloat() const { return bloat_; }
...
//// start out with non-terminating mesh lines
  ...
  bool operator<(const MeshLine2d &mp) const { return position < mp.position; }

  double getPosition() { return position; }
  double getPositiveSpacing() { return posSpacing; }
  double getNegativeSpacing() { return negSpacing; }
...
class MeshRegion2d {
public:
  MeshRegion2d(const std::string & /*region*/,
...
public:
  ...
  MeshInterface2d(const std::string & /*name*/, const std::string & /*region0*/,
                  const std::string & /*region1*/);
  const std::string &GetName() const { return name; }
  const std::string &GetRegion0() const { return Region0; }
  const std::string &GetRegion1() const { return Region1; }

  bool IsPointInside(double /*x*/, double /*y*/) const;

  void AddBoundingBox(const MeshInterface2d &);
  void AddBoundingBox(const BoundingBox &bb) { bboxes_.push_back(bb); }
...
public:
  //// We will use overlaps to detect interface connections
  MeshContact2d(const std::string & /*name*/, const std::string & /*material*/,
...
//// If we have coordinates which don't become material, do we create them with
///default region, or do we just not assign them to the final structure.
} // namespace dsMesh
#endif
