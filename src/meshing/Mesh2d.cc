...
bool Mesh2d::Instantiate_(const std::string &deviceName,
...
MeshRegion2dPtr Mesh2d::FindRegion(double x, double y) {
  ...
  for (std::vector<std::string>::iterator it = regionOrder.begin();
       it != regionOrder.end(); ++it) {
...
bool Mesh2d::Finalize_(std::string &errorString) {
  ...
  //// remove collinear points
  {
    ...
    for (size_t x = 1; x < xlen; ++x) {
      if (xlines[x]->getPosition() == xlines[x - 1]->getPosition()) {
        os << "Removing collinear point from meshlines in x direction at "
           << xlines[x]->getPosition() << "\n";
      } else {
    ...
    for (size_t y = 1; y < ylen; ++y) {
      if (ylines[y]->getPosition() == ylines[y - 1]->getPosition()) {
        os << "Removing collinear point from meshlines in y direction at "
           << ylines[y]->getPosition() << "\n";
      } else {
  ...
  for (std::vector<std::string>::iterator rt = regionOrder.begin();
       rt != regionOrder.end(); ++rt) {
    CoordinateToNode[*rt] = std::vector<size_t>(coordinates.size(), size_t(-1));
    MaxNodeIndex[*rt] = 0;
  }

  for (std::vector<MeshTriangle>::iterator it = triangles.begin();
       it != triangles.end(); ++it) {
...
#if 1
      std::ostringstream os;
      os << "Triangle has no region: " << coordinates[i0].GetVector() << " "
         << coordinates[i1].GetVector() << " " << coordinates[i2].GetVector()
         << " " << v0 << "\n";
      OutputStream::WriteOut(OutputStream::OutputType::INFO, os.str().c_str());
#endif
    }
  ...
  for (size_t i = 0; i < nodesAtCoordinate.size(); ++i) {
    ...
    if (count > 1) {
      ...
      bool found = false;
      for (std::vector<std::string>::iterator ci = contactOrder.begin();
           ci != contactOrder.end(); ++ci) {
        ...
        if (cn != CoordinateToNode.end()) {
          ...
          if (ni != size_t(-1)) {
            ...
            found = true;
            std::ostringstream os;
            os << "Contact: " << nm << " region " << r << " ni: " << ni
               << " ci: " << i << " " << coordinates[i].GetVector() << "\n";
            OutputStream::WriteOut(OutputStream::OutputType::INFO,
                                   os.str().c_str());
            break;
          }
      ...
      if (!found) {
        for (std::vector<std::string>::iterator ii = interfaceOrder.begin();
             ii != interfaceOrder.end(); ++ii) {
          // TODO: Check Bounding box
          ...
          if ((in0 != CoordinateToNode.end()) &&
              (in1 != CoordinateToNode.end())) {
            size_t ni0 = (in0->second)[i];
            size_t ni1 = (in1->second)[i];
            if ((ni0 != size_t(-1)) && (ni1 != size_t(-1))) {
              meshLoader->GetMeshInterface(nm).AddNodePair(
                  MeshInterfaceNodePair(ni0, ni1));
              found = true;
              break;
            }
          } else {
...
void Mesh2d::AddRegion(MeshRegion2dPtr mreg) {
  ...
  if (it != regions.end()) {
    ...
    if (morig.GetMaterial() != mreg->GetMaterial()) {
      std::ostringstream os;
      os << "Not changing material from " << morig.GetMaterial() << " to "
         << mreg->GetMaterial() << " for region " << nm << "\n";
      OutputStream::WriteOut(OutputStream::OutputType::INFO, os.str().c_str());
    }
...
void Mesh2d::AddInterface(MeshInterface2dPtr mint) {
  ...
  if (it != interfaces.end()) {
    ...
    if ((morig.GetRegion0() != mint->GetRegion0()) ||
...
void Mesh2d::AddContact(MeshContact2dPtr mcon) {
  ...
  if (it != contacts.end()) {
    ...
    if ((morig.GetRegion() != mcon->GetRegion())) {
      std::ostringstream os;
