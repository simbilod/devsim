...
public:
  ...
  Solution(const std::string &n)
      : name(n), model_type(ModelType::MUNDEFINED),
        data_type(DataType::DUNDEFINED), uniform_value(0.0), reserve_size(0) {}

  bool HasValues() { return !values.empty(); }

  const std::string &GetName() { return name; }

  const std::string &GetCommandName() const { return command_name; }

  const std::vector<std::pair<std::string, ObjectHolder>> &
  GetCommandArgs() const {
    return command_args;
  }

  void SetCommandName(const std::string &p) { command_name = p; }
  ...
  void SetParent(const std::string &p) { parent = p; }

  void AddValue(double x) { values.push_back(x); }

  ModelType GetModelType() const { return model_type; }

  void SetModelType(ModelType mt) { model_type = mt; }
  ...
  double GetUniformValue() const { return uniform_value; }

  void SetUniformValue(double v) { uniform_value = v; }

  typedef std::vector<double> values_t;

  const values_t &GetValues() const { return values; }

  void SetReserve(size_t rs) { reserve_size = rs; }
...
public:
  ...
  size_t Index0() const { return index0; }
  size_t Index1() const { return index1; }
...
public:
  ...
  bool operator==(const MeshTriangle &mt) const {
    return (this->index0 == mt.Index0()) && (this->index1 == mt.Index1()) &&
  ...
  size_t Index0() const { return index0; }
  size_t Index1() const { return index1; }
  size_t Index2() const { return index2; }
...
class MeshTetrahedron {
public:
  MeshTetrahedron(size_t i, size_t j, size_t k, size_t l)
      : index0(i), index1(j), index2(k), index3(l) {
    std::vector<size_t> vec;
    vec.reserve(4);
    vec.push_back(i);
    vec.push_back(j);
  ...
  bool operator==(const MeshTetrahedron &mt) const {
    return (this->index0 == mt.Index0()) && (this->index1 == mt.Index1()) &&
           (this->index2 == mt.Index2()) && (this->index3 == mt.Index3());
  }
  ...
  size_t Index1() const { return index1; }
  size_t Index2() const { return index2; }
  size_t Index3() const { return index3; }
...
public:
  ...
  Vector<double> GetVector() const { return Vector<double>(xpos, ypos, zpos); }
...
class MeshRegion {
public:
  MeshRegion(const std::string &n, const std::string &m)
      : name(n), material(m) {
    nodes.reserve(1000);
    edges.reserve(1000);
  }
  ...
  void AddNode(const MeshNode &mn) { nodes.push_back(mn); }

  void AddEdge(const MeshEdge &me) { edges.push_back(me); }

  void AddTriangle(const MeshTriangle &mt) { triangles.push_back(mt); }

  void AddTetrahedron(const MeshTetrahedron &mt) { tetrahedra.push_back(mt); }

  bool HasNodes() const { return !nodes.empty(); }

  bool HasEdges() const { return !edges.empty(); }

  bool HasTriangles() const { return !triangles.empty(); }

  bool HasTetrahedra() const { return !tetrahedra.empty(); }

  const MeshNodeList_t &GetNodes() const { return nodes; }

  const MeshEdgeList_t &GetEdges() const { return edges; }

  const MeshTriangleList_t &GetTriangles() const { return triangles; }

  const MeshTetrahedronList_t &GetTetrahedra() const { return tetrahedra; }

  bool IsSolution(const std::string &n) { return solutionList.count(n) != 0; }

  void AddSolution(SolutionPtr sp) { solutionList[sp->GetName()] = sp; }

  const MeshSolutionList_t &GetSolutionList() const { return solutionList; }

  bool IsEquation(const std::string &n) { return equationList.count(n) != 0; }

  void AddEquation(EquationPtr sp) { equationList[sp->GetName()] = sp; }

  const MeshEquationList_t &GetEquationList() const { return equationList; }
...
public:
  ...
  void SetIndex0(size_t i) { index0 = i; }

  void SetIndex1(size_t i) { index1 = i; }
...
class MeshInterface {
public:
  MeshInterface(const std::string &n, const std::string &r0,
                const std::string &r1)
      : name(n), region0(r0, ""), region1(r1, "") {}

  ~MeshInterface();

  void AddNodePair(const MeshInterfaceNodePair &np) { nodePairs.push_back(np); }
  ...
  bool HasNodesPairs() const { return !nodePairs.empty(); }

  bool HasEdges() const { return region0.HasEdges(); }

  bool HasTriangles() const { return region0.HasTriangles(); }

  const std::string &GetName() const { return name; }
  const std::string &GetRegion0() const { return region0.GetName(); }

  const std::string &GetRegion1() const { return region1.GetName(); }

  typedef std::vector<MeshInterfaceNodePair> NodePairList_t;
  const NodePairList_t &GetNodePairs() { return nodePairs; }

  const MeshEdgeList_t &GetEdges0() const { return region0.GetEdges(); }

  const MeshEdgeList_t &GetEdges1() const { return region1.GetEdges(); }
  ...
  MeshRegion &GetMeshRegion0() { return region0; }

  MeshRegion &GetMeshRegion1() { return region1; }
...
class MeshContact {
public:
  MeshContact(const std::string &n, const std::string &r, const std::string &m)
      : name(n), region(r, ""), material(m) {}

  ~MeshContact();

  void AddNode(const MeshNode &n) { region.AddNode(n); }

  void AddEdge(const MeshEdge &e) { region.AddEdge(e); }

  void AddTriangle(const MeshTriangle &t) { region.AddTriangle(t); }

  bool HasNodes() const { return region.HasNodes(); }

  bool HasEdges() const { return region.HasEdges(); }

  bool HasTriangles() { return region.HasTriangles(); }

  const std::string &GetName() const { return name; }

  const std::string &GetRegion() const { return region.GetName(); }

  const std::string &GetMaterial() const { return material; }

  const MeshNodeList_t &GetNodes() { return region.GetNodes(); }

  const MeshEdgeList_t &GetEdges() { return region.GetEdges(); }

  const MeshTriangleList_t &GetTriangles() { return region.GetTriangles(); }
  ...
  MeshRegion &GetMeshRegion() { return region; }
