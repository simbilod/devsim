...
struct GmshElement {
  GmshElement(size_t ei, size_t pn, Shapes::ElementType_t et,
              const std::vector<int> &ni)
      : element_index(ei), physical_number(pn), element_type(et),
        node_indexes(ni) {
    /* this is an optimization as we now currently allow only one element type
     * per dimension */
...
struct GmshInterfaceInfo {
  GmshInterfaceInfo(const std::string &iname, const std::string &rname0,
                    const std::string &rname1)
      : interface(iname), region0(rname0), region1(rname1) {}
  GmshInterfaceInfo() {}
...
struct GmshContactInfo {
  GmshContactInfo(const std::string &cname, const std::string &rname,
                  const std::string &mname)
      : contact(cname), region(rname), material(mname) {}
  GmshContactInfo() {}
...
struct GmshRegionInfo {
  GmshRegionInfo(const std::string &rname, const std::string &mname)
      : region(rname), material(mname) {}
  GmshRegionInfo() {}
...
public:
  ...
  void AddElement(const GmshElement &ge) { elementList.push_back(ge); }

  void MapPhysicalNameToContact(const std::string &pname,
  ...
  void MapPhysicalNameToInterface(const std::string &pname,
                                  const std::string &iname,
                                  const std::string &rname0,
                                  const std::string &rname1) {
    if (!pname.empty()) {
  ...
  void MapPhysicalNameToRegion(const std::string &pname,
                               const std::string &rname,
                               const std::string &mname) {
    if (!pname.empty()) {
...
private:
  ...
  void GetUniqueNodesFromPhysicalNames(const std::vector<std::string> &,
                                       MeshNodeList_t &);
  void GetUniqueTetrahedraFromPhysicalNames(const std::vector<std::string> &,
                                            MeshTetrahedronList_t &);
  void GetUniqueTrianglesFromPhysicalNames(const std::vector<std::string> &,
                                           MeshTriangleList_t &);
  void GetUniqueEdgesFromPhysicalNames(const std::vector<std::string> &,
                                       MeshEdgeList_t &);
