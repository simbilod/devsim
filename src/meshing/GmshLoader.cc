...
namespace {
void processNodes(const MeshNodeList_t &mnlist,
                  const std::vector<Coordinate *> &clist,
...
void processEdges(const MeshEdgeList_t &tl, const std::vector<Node *> &nlist,
                  std::vector<const Edge *> &edge_list) {
...
void processTriangles(const MeshTriangleList_t &tl,
                      const std::vector<Node *> &nlist,
...
void processTetrahedra(const MeshTetrahedronList_t &tl,
                       const std::vector<Node *> &nlist,
...
GmshLoader::GmshLoader(const std::string &n)
    : Mesh(n), dimension(0), maxCoordinateIndex(0) {
  //// Arbitrary number
...
void GmshLoader::GetUniqueNodesFromPhysicalNames(
    const std::vector<std::string> &pnames, MeshNodeList_t &mnlist) {
  mnlist.clear();
  for (std::vector<std::string>::const_iterator pit = pnames.begin();
       pit != pnames.end(); ++pit) {
    const std::string &pname = *pit;

    const MeshNodeList_t &pnlist = gmshShapesMap[pname].Points;

    for (MeshNodeList_t::const_iterator nit = pnlist.begin();
         nit != pnlist.end(); ++nit) {
...
void GmshLoader::GetUniqueTetrahedraFromPhysicalNames(
    const std::vector<std::string> &pnames, MeshTetrahedronList_t &mnlist) {
  mnlist.clear();
  for (std::vector<std::string>::const_iterator pit = pnames.begin();
       pit != pnames.end(); ++pit) {
    const std::string &pname = *pit;
    ...
    for (MeshTetrahedronList_t::const_iterator nit = pnlist.begin();
         nit != pnlist.end(); ++nit) {
      mnlist.push_back(*nit);
    }
  }
  /// Remove overlapping nodes
  std::sort(mnlist.begin(), mnlist.end());
  MeshTetrahedronList_t::iterator elnewend =
      std::unique(mnlist.begin(), mnlist.end());
  mnlist.erase(elnewend, mnlist.end());
}

void GmshLoader::GetUniqueTrianglesFromPhysicalNames(
    const std::vector<std::string> &pnames, MeshTriangleList_t &mnlist) {
  mnlist.clear();
  for (std::vector<std::string>::const_iterator pit = pnames.begin();
       pit != pnames.end(); ++pit) {
    const std::string &pname = *pit;
    ...
    for (MeshTriangleList_t::const_iterator nit = pnlist.begin();
         nit != pnlist.end(); ++nit) {
      mnlist.push_back(*nit);
    }
  }
  /// Remove overlapping nodes
  std::sort(mnlist.begin(), mnlist.end());
  MeshTriangleList_t::iterator elnewend =
      std::unique(mnlist.begin(), mnlist.end());
  mnlist.erase(elnewend, mnlist.end());
}

void GmshLoader::GetUniqueEdgesFromPhysicalNames(
    const std::vector<std::string> &pnames, MeshEdgeList_t &mnlist) {
  mnlist.clear();
  for (std::vector<std::string>::const_iterator pit = pnames.begin();
       pit != pnames.end(); ++pit) {
    const std::string &pname = *pit;
    ...
    for (MeshEdgeList_t::const_iterator nit = pnlist.begin();
         nit != pnlist.end(); ++nit) {
      mnlist.push_back(*nit);
    }
  }
...
bool GmshLoader::Instantiate_(const std::string &deviceName,
  ...
  size_t ccount = 0;
  for (MeshCoordinateList_t::const_iterator it = meshCoordinateList.begin();
       it != meshCoordinateList.end(); ++it) {
    const MeshCoordinate &mc = (it->second);
    CoordinatePtr new_coordinate =
        new Coordinate(mc.GetX(), mc.GetY(), mc.GetZ());
  ...
  {
    std::ostringstream os;
    os << "Device " << deviceName << " has " << ccount
       << " coordinates with max index " << maxCoordinateIndex << "\n";
    OutputStream::WriteOut(OutputStream::OutputType::INFO, os.str());
  }

  std::map<std::string, std::vector<NodePtr>> RegionNameToNodeMap;

  //// For each name in the region map, we create a list of nodes which are
  ///indexes
  [this, &RegionNameToNodeMap, dp, &coordinate_list]() {
    ...
    for (MapToRegionInfo_t::const_iterator rit = regionMap.begin();
         rit != regionMap.end(); ++rit) {
      mesh_nodes.clear();
      ...
      size_t ncount = 0;
      for (std::vector<NodePtr>::const_iterator nit = nodeList.begin();
  ...
  [this, &RegionNameToNodeMap, dp, &ret]() {
    ...
    for (MapToContactInfo_t::const_iterator cit = contactMap.begin();
         cit != contactMap.end(); ++cit) {
      mesh_nodes.clear();
      mesh_edges.clear();
      mesh_triangles.clear();
      ...
      if (!RegionNameToNodeMap.count(regionName)) {
        ...
        os << "Contact " << contactName
           << " references non-existent region name " << regionName << ".\n";
      ...
      if (dimension == 2 && !mesh_triangles.empty()) {
        std::ostringstream os;
        os << "Contact " << contactName << " region name " << regionName
           << " must be 1 dimensional to be a contact.\n";
        OutputStream::WriteOut(OutputStream::OutputType::ERROR, os.str());
        ret = false;
        ...
        std::ostringstream os;
        os << "Contact " << contactName << " region name " << regionName
           << " must be 0 dimensional to be a contact.\n";
        OutputStream::WriteOut(OutputStream::OutputType::ERROR, os.str());
        ret = false;
      }
      ...
      if (contactptr) {
        std::ostringstream os;
        os << "Contact " << contactName << " on region " << regionName
      ...
      cedges.clear();
      ctriangles.clear();
      for (MeshNodeList_t::const_iterator tnit = mesh_nodes.begin();
           tnit != mesh_nodes.end(); ++tnit) {
        size_t cindex = tnit->Index();

        if (cindex > maxCoordinateIndex) {
          std::ostringstream os;
          os << "Contact " << contactName << " reference coordinate " << cindex
             << " which does not exist on any region.\n";
          OutputStream::WriteOut(OutputStream::OutputType::ERROR, os.str());
          ret = false;
          continue;
        }
      ...
      if (cnodes.empty()) {
        std::ostringstream os;
        os << "Contact " << contactName
           << " does not reference any nodes on region " << regionName
           << " .\n";
        OutputStream::WriteOut(OutputStream::OutputType::ERROR, os.str());
        ret = false;
        continue;
      ...
      for (MeshEdgeList_t::const_iterator tnit = mesh_edges.begin();
           tnit != mesh_edges.end(); ++tnit) {
        mesh_edge_nodes[0] = nodeList[tnit->Index0()];
        mesh_edge_nodes[1] = nodeList[tnit->Index1()];
        ConstEdgePtr tedge =
            regionptr->FindEdge(mesh_edge_nodes[0], mesh_edge_nodes[1]);
        if (tedge) {
      ...
      for (MeshTriangleList_t::const_iterator tnit = mesh_triangles.begin();
           tnit != mesh_triangles.end(); ++tnit) {
        ...
        ConstTrianglePtr ttriangle = regionptr->FindTriangle(
            mesh_triangle_nodes[0], mesh_triangle_nodes[1],
            mesh_triangle_nodes[2]);
        if (ttriangle) {
      ...
      cp->AddEdges(cedges);
      std::ostringstream os;
      os << "Contact " << contactName << " in region " << regionName << " with "
         << cnodes.size() << " nodes"
         << "\n";
      OutputStream::WriteOut(OutputStream::OutputType::INFO, os.str());
    }
  ...
  [this, &RegionNameToNodeMap, dp, &ret]() {
    ...
    MeshEdgeList_t mesh_edges;
    MeshTriangleList_t mesh_triangles;
    for (MapToInterfaceInfo_t::const_iterator iit = interfaceMap.begin();
      ...
      if (!RegionNameToNodeMap.count(regionName0)) {
        ...
        os << "Interface " << interfaceName
           << " references non-existent region name " << regionName0 << ".\n";
        OutputStream::WriteOut(OutputStream::OutputType::ERROR, os.str());
        ret = false;
        continue;
      }
      if (!RegionNameToNodeMap.count(regionName1)) {
        std::ostringstream os;
        os << "Interface " << interfaceName
           << " references non-existent region name " << regionName1 << ".\n";
        OutputStream::WriteOut(OutputStream::OutputType::ERROR, os.str());
        ret = false;
        continue;
      }
      ...
      if (interfaceptr) {
        std::ostringstream os;
        os << "Interface " << interfaceName << " on regions " << regionName0
           << " and " << regionName1 << " being instantiated multiple times.\n";
        OutputStream::WriteOut(OutputStream::OutputType::ERROR, os.str());
        ret = false;
        continue;
      }
      ...
      if (dimension == 2 && !mesh_triangles.empty()) {
        ...
        os << "Interface " << interfaceName << " on regions " << regionName0
           << " and " << regionName1
           << " must be 1 dimensional to be interface.\n";
        OutputStream::WriteOut(OutputStream::OutputType::ERROR, os.str());
        ret = false;
        ...
        os << "Interface " << interfaceName << " on regions " << regionName0
           << " and " << regionName1
           << " must be 0 dimensional to be interface.\n";
        OutputStream::WriteOut(OutputStream::OutputType::ERROR, os.str());
        ret = false;
      }
      ...
      for (MeshNodeList_t::const_iterator tnit = mesh_nodes.begin();
        ...
        if (iindex > maxCoordinateIndex) {
          std::ostringstream os;
          os << "Interface " << interfaceName << " reference coordinate "
             << iindex << " which does not exist on any region.\n";
          OutputStream::WriteOut(OutputStream::OutputType::ERROR, os.str());
          ret = false;
          continue;
        }
        ...
        if (np[0] && np[1]) {
          ...
          if (!np[0]) {
            os << "Interface " << interfaceName << " reference coordinate "
               << iindex << " not on region " << regionName0 << ".\n";
          }
          if (!np[1]) {
            os << "Interface " << interfaceName << " reference coordinate "
               << iindex << " not on region " << regionName1 << ".\n";
          }
          OutputStream::WriteOut(OutputStream::OutputType::ERROR, os.str());
          ret = false;
          continue;
      ...
      if (inodes[0].empty()) {
        std::ostringstream os;
        os << "Interface " << interfaceName
           << " does not reference any nodes on regions " << regionName0
           << " and " << regionName1 << " .\n";
        OutputStream::WriteOut(OutputStream::OutputType::ERROR, os.str());
        ret = false;
        continue;
      ...
      for (MeshEdgeList_t::const_iterator tnit = mesh_edges.begin();
        ...
        tedge[0] =
            regionptr[0]->FindEdge(mesh_edge_nodes[0], mesh_edge_nodes[1]);

        mesh_edge_nodes[0] = nodeList1[tnit->Index0()];
        mesh_edge_nodes[1] = nodeList1[tnit->Index1()];
        tedge[1] =
            regionptr[1]->FindEdge(mesh_edge_nodes[0], mesh_edge_nodes[1]);
      ...
      for (MeshTriangleList_t::const_iterator tnit = mesh_triangles.begin();
        ...
        ttriangle[0] = regionptr[0]->FindTriangle(mesh_triangle_nodes[0],
                                                  mesh_triangle_nodes[1],
                                                  mesh_triangle_nodes[2]);
        ...
        ttriangle[1] = regionptr[1]->FindTriangle(mesh_triangle_nodes[0],
                                                  mesh_triangle_nodes[1],
                                                  mesh_triangle_nodes[2]);
      ...
      Interface *iptr = new Interface(interfaceName, regionptr[0], regionptr[1],
                                      inodes[0], inodes[1]);
      ...
      std::ostringstream os;
      os << "Adding interface " << interfaceName << " with " << inodes[0].size()
         << ", " << inodes[1].size() << " nodes"
         << "\n";
      OutputStream::WriteOut(OutputStream::OutputType::INFO, os.str());
    }
...
bool GmshLoader::Finalize_(std::string &errorString) {
  ...
  for (size_t i = 0; i < elementList.size(); ++i) {
    ...
    const PhysicalIndexToName_t &physicalIndexNameMap =
        physicalDimensionIndexNameMap[dimension];
    PhysicalIndexToName_t::const_iterator pit =
        physicalIndexNameMap.find(physical_number);
  ...
  for (ShapesMap_t::iterator it = gmshShapesMap.begin();
       it != gmshShapesMap.end(); ++it) {
    Shapes &shapes = it->second;

    if (shapes.GetNumberOfTypes() != 1) {
      ret = false;
      std::ostringstream os;
      os << "Physical group name " << it->first
         << " has multiple element types.\n";
      OutputStream::WriteOut(OutputStream::OutputType::ERROR, os.str());
    ...
    {
      std::ostringstream os;
      os << "Physical group name " << it->first << " has "
         << shapes.Tetrahedra.size() << " Tetrahedra.\n";
      os << "Physical group name " << it->first << " has "
         << shapes.Triangles.size() << " Triangles.\n";
      os << "Physical group name " << it->first << " has "
         << shapes.Lines.size() << " Lines.\n";
      os << "Physical group name " << it->first << " has "
         << shapes.Points.size() << " Points.\n";
      OutputStream::WriteOut(OutputStream::OutputType::INFO, os.str());
    }
  }
...
bool GmshLoader::HasPhysicalName(const size_t d, const size_t i) const {
  bool ret = false;
  const PhysicalIndexToName_t &physicalIndexNameMap =
      physicalDimensionIndexNameMap[d];
