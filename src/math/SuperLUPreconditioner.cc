...
SuperLUPreconditioner<DoubleType>::SuperLUPreconditioner(
    size_t sz, PEnum::TransposeType_t transpose, PEnum::LUType_t lutype)
    : Preconditioner<DoubleType>(sz, transpose), superLUData_(nullptr),
      lutype_(lutype) {}

template <typename DoubleType>
dsMath::CompressionType
SuperLUPreconditioner<DoubleType>::GetRealMatrixCompressionType() const {
  return dsMath::CompressionType::CCM;
}

template <typename DoubleType>
dsMath::CompressionType
SuperLUPreconditioner<DoubleType>::GetComplexMatrixCompressionType() const {
  return dsMath::CompressionType::CCM;
}
...
bool SuperLUPreconditioner<DoubleType>::DerivedLUFactor(Matrix<DoubleType> *m) {
  ...
  if (!superLUData_) {
    if (lutype_ == PEnum::LUType_t::FULL) {
      superLUData_ =
          new SuperLUData(Preconditioner<DoubleType>::size(),
                          Preconditioner<DoubleType>::GetTransposeSolve(),
                          PEnum::LUType_t::FULL);
    } else if (lutype_ == PEnum::LUType_t::INCOMPLETE) {
      superLUData_ =
          new SuperLUData(Preconditioner<DoubleType>::size(),
                          Preconditioner<DoubleType>::GetTransposeSolve(),
                          PEnum::LUType_t::INCOMPLETE);
    } else {
...
template <typename DoubleType>
void SuperLUPreconditioner<DoubleType>::DerivedLUSolve(
