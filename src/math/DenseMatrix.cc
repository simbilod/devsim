...
#ifdef USE_LAPACK
template <typename T> struct matrix_data_lapack {
  ...
  T &operator()(size_t r, size_t c) {
    ...
    // probably not important for our application since it is only done once and
    // the LUFactors are stored
    return A_[r + c * dim_];
  }

  bool LUFactor() {
    getrf(&dim_, &dim_, reinterpret_cast<T *>(&A_[0]), &dim_, &ipiv_[0],
          &info_);
    factored_ = true;
    return (info_ == 0);
  }
  ...
  bool Solve(T *B) {
    ...
    if (info_ == 0) {
      getrs(&trans, &dim_, &nrhs, reinterpret_cast<T *>(&A_[0]), &dim_,
...
template <typename T> struct matrix_data_eigen {
  ...
  T &operator()(size_t r, size_t c) { return A_(r, c); }
...
template <typename T> DenseMatrix<T>::DenseMatrix(size_t d) {
  matrixdata_ = std::make_unique<matrix_data<T>>(d);
}

template <typename T> DenseMatrix<T>::~DenseMatrix() = default;
...
template <>
struct dsMath::matrix_data<double> : public matrix_data_eigen<double> {
  using matrix_data_eigen<double>::matrix_data_eigen;
};
#elif defined(USE_LAPACK)
template <>
struct dsMath::matrix_data<double> : public matrix_data_lapack<double> {
  using matrix_data_lapack<double>::matrix_data_lapack;
};
#else
...
template <>
struct dsMath::matrix_data<float128> : public matrix_data_eigen<float128> {
  using matrix_data_eigen<float128>::matrix_data_eigen;
};
#elif defined(USE_LAPACK)
template <>
struct dsMath::matrix_data<float128> : public matrix_data_lapack<float128> {
  using matrix_data_lapack<float128>::matrix_data_lapack;
};
#else
#error "no dense matrix function"
#endif
