...
CompressedMatrix<DoubleType>::CompressedMatrix(size_t sz, MatrixType mt,
                                               CompressionType ct)
    : Matrix<DoubleType>(sz), matType_(mt), compressionType_(ct),
      compressed(false), symbolicstatus_(SymbolicStatus_t::NEW_SYMBOLIC) {
...
template <typename DoubleType>
const dsMath::DoubleVec_t<DoubleType> &
CompressedMatrix<DoubleType>::GetReal() const {
  dsAssert(compressed, "UNEXPECTED");
...
template <typename DoubleType>
const dsMath::DoubleVec_t<DoubleType> &
CompressedMatrix<DoubleType>::GetImag() const {
  dsAssert(compressed, "UNEXPECTED");
...
/// Model this after
template <typename DoubleType>
void CompressedMatrix<DoubleType>::AddImagEntryImpl(int r, int c,
                                                    DoubleType v) {
#ifndef NDEBUG
...
template <typename DoubleType> void CompressedMatrix<DoubleType>::Finalize() {
  if (!compressed) {
