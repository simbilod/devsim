...
template <typename T>
dsMath::Preconditioner<T> *
CreateExternalPreconditioner(size_t numeqns,
                             dsMath::PEnum::TransposeType_t transtype,
...
CreateACPreconditioner(PEnum::TransposeType_t trans_type, size_t numeqns) {
...
template <typename DoubleType>
CompressedMatrix<DoubleType> *
CreateMatrix(Preconditioner<DoubleType> *preconditioner, bool is_complex) {
...
template <typename DoubleType>
CompressedMatrix<DoubleType> *
CreateACMatrix(Preconditioner<DoubleType> *preconditioner) {
  return CreateMatrix(preconditioner, true);
}

template Preconditioner<double> *CreateDirectPreconditioner(size_t numeqns);
template Preconditioner<double> *
CreatePreconditioner(LinearSolver<double> &itermethod, size_t numeqns);
template Preconditioner<double> *
CreateACPreconditioner(PEnum::TransposeType_t trans_type, size_t numeqns);
template CompressedMatrix<double> *
CreateMatrix(Preconditioner<double> *preconditioner, bool is_complex);
template CompressedMatrix<double> *
CreateACMatrix(Preconditioner<double> *preconditioner);
#ifdef DEVSIM_EXTENDED_PRECISION
template Preconditioner<float128> *CreateDirectPreconditioner(size_t numeqns);
template Preconditioner<float128> *
CreatePreconditioner(LinearSolver<float128> &itermethod, size_t numeqns);
template Preconditioner<float128> *
CreateACPreconditioner(PEnum::TransposeType_t trans_type, size_t numeqns);
template CompressedMatrix<float128> *
CreateMatrix(Preconditioner<float128> *preconditioner, bool is_complex);
template CompressedMatrix<float128> *
CreateACMatrix(Preconditioner<float128> *preconditioner);
#endif
} // namespace dsMath
