...
public:
  ...
  bool LUSolve(DoubleVec_t<DoubleType> &x,
               const DoubleVec_t<DoubleType> &b) const;
  bool LUSolve(ComplexDoubleVec_t<DoubleType> &x,
...
protected:
  virtual void DerivedLUSolve(DoubleVec_t<DoubleType> &x,
                              const DoubleVec_t<DoubleType> &b) const = 0;
  virtual void
  DerivedLUSolve(ComplexDoubleVec_t<DoubleType> &x,
                 const ComplexDoubleVec_t<DoubleType> &b) const = 0;
  virtual bool DerivedLUFactor(Matrix<DoubleType> *) = 0; // Factor the matrix

  Matrix<DoubleType> &GetMatrix() { return *matrix_; }
