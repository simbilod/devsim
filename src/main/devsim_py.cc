...
void devsim_initialization() {
  ...
  features["copyright"] =
      ObjectHolder(u8"Copyright \u00a9 " DEVSIM_COPYRIGHT_YEAR " DEVSIM LLC");
  ...
  if (ret == MathLoader::LoaderMessages_t::MKL_LOADED) {
    ...
    os << "Error loading math libraries.  Please install a suitable "
          "BLAS/LAPACK library and set DEVSIM_MATH_LIBS.  Alternatively, "
          "install the Intel MKL.\n";
    OutputStream::WriteOut(OutputStream::OutputType::INFO, os.str());
    OutputStream::WriteOut(OutputStream::OutputType::FATAL, errors);
  }
  features["math_libraries"] =
      CreateObjectHolderList(MathLoader::GetLoadedMathDLLs());
