...
Triangle::Triangle(size_t ind, ConstNodePtr n0, ConstNodePtr n1,
                   ConstNodePtr n2)
    : nodes(3) {
  index = ind;
  nodes[0] = n0;
...
const std::vector<ConstNodePtr> &Triangle::GetFENodeList() const {
  ...
  static const Vector<double> pdirs[] = {Vector<double>(0.0, 0.0, 1.0),
...
Vector<DoubleType> GetCenter(const Triangle &tr) {
  ...
  const Vector<DoubleType> &v1 =
      ConvertPosition<DoubleType>(nodes[1]->Position());
  const Vector<DoubleType> &v2 =
      ConvertPosition<DoubleType>(nodes[2]->Position());
...
template <typename DoubleType>
Vector<DoubleType> GetTriangleCenter3d(const Vector<DoubleType> &p1,
                                       const Vector<DoubleType> &p2,
                                       const Vector<DoubleType> &p3) {
...
template <typename DoubleType>
Vector<DoubleType> GetTriangleCenterInPlane(DoubleType x0, DoubleType x1,
                                            DoubleType x2, DoubleType y0,
                                            DoubleType y1, DoubleType y2) {
...
template <typename DoubleType>
Vector<DoubleType> GetTriangleCenter(const Vector<DoubleType> &v0,
                                     const Vector<DoubleType> &v1,
                                     const Vector<DoubleType> &v2) {
  ...
  if ((abs(v10.Getz()) < Triangle::EPSILON) &&
      (abs(v20.Getz()) < Triangle::EPSILON)) {
    Vector<DoubleType> temp = GetTriangleCenterInPlane(
        v0.Getx(), v1.Getx(), v2.Getx(), v0.Gety(), v1.Gety(), v2.Gety());
    ret = Vector<DoubleType>(temp.Getx(), temp.Gety(), v0.Getz());
  } else if ((abs(v10.Gety()) < Triangle::EPSILON) &&
             (abs(v20.Gety()) < Triangle::EPSILON)) {
    Vector<DoubleType> temp = GetTriangleCenterInPlane(
        v0.Getx(), v1.Getx(), v2.Getx(), v0.Getz(), v1.Getz(), v2.Getz());
    ret = Vector<DoubleType>(temp.Getx(), v0.Gety(), temp.Gety());
  } else if ((abs(v10.Getx()) < Triangle::EPSILON) &&
             (abs(v20.Getx()) < Triangle::EPSILON)) {
    Vector<DoubleType> temp = GetTriangleCenterInPlane(
        v0.Getz(), v1.Getz(), v2.Getz(), v0.Gety(), v1.Gety(), v2.Gety());
