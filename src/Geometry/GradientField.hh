...
/*
 * This is owned by the region.  It will be evaluated the first time it is
 * accessed through the region
 *
 * It holds all of the dense matrix structures with the LU factors for finding
 * the gradient on a triangle
 */
#include "Vector.hh"
...
template <typename DoubleType> class GradientField {
...
private:
  const Region &GetRegion() const { return *myregion_; }
  GradientField();
