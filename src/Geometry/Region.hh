...
template <typename T> class GradientField;
template <typename T> class TriangleElementField;
template <typename T> class TetrahedronElementField;

template <typename T> class ObjectCache;
namespace MEE {
template <typename DoubleType> class ModelExprData;
}
...
using ConstTetrahedronEdgeModelPtr =
    std::shared_ptr<const TetrahedronEdgeModel>;
...
public:
  ...
  typedef std::map<std::string, TetrahedronEdgeModelPtr>
      TetrahedronEdgeModelList_t;
  ...
  size_t GetNumberNodes() const { return nodeList.size(); }

  size_t GetNumberEdges() const { return edgeList.size(); }

  size_t GetNumberTriangles() const { return triangleList.size(); }

  size_t GetNumberTetrahedrons() const { return tetrahedronList.size(); }

  size_t GetDimension() const { return dimension; }
  const std::string &GetName() const { return regionName; }
  ...
  const ConstEdgeList &GetEdgeList() const { return edgeList; }

  const ConstNodeList &GetNodeList() const { return nodeList; }

  const ConstTriangleList &GetTriangleList() const { return triangleList; }
  ...
  ConstTetrahedronPtr FindTetrahedron(ConstNodePtr, ConstNodePtr, ConstNodePtr,
                                      ConstNodePtr) const;
  ...
  GetTetrahedronEdgeModel(const std::string &) const;
  ...
  const EdgeModelList_t &GetEdgeModelList() const { return edgeModels; }

  const NodeModelList_t &GetNodeModelList() const { return nodeModels; }
  ...
  template <typename DoubleType> DoubleType GetAbsError() const {
    return static_cast<DoubleType>(absError);
  }

  template <typename DoubleType> DoubleType GetRelError() const {
  ...
  template <typename DoubleType>
  void NoiseUpdate(const std::string & /*output*/,
...
private:
  ...
  template <typename DoubleType> struct GeometryField {
    GeometryField()
        : gradientField(nullptr), triangleElementField(nullptr),
          tetrahedronElementField(nullptr) {}
