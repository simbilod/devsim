...
template <typename DoubleType> struct TriangleElementFieldMatrixHolder {
...
template <typename DoubleType> class TriangleElementField {
...
public:
  ...
  void GetTriangleElementField(const Triangle &, const TriangleEdgeModel &,
                               const TriangleEdgeModel &,
                               EdgeVectors_t &) const;
  void GetTriangleElementField(const Triangle &, const TriangleEdgeModel &,
                               const EdgeModel &, EdgeVectors_t &) const;
  void GetTriangleElementField(const Triangle &, const TriangleEdgeModel &,
                               const EdgeModel &, const EdgeModel &,
                               DerivativeEdgeVectors_t &) const;

  void GetTriangleElementFieldPairs(const Triangle &, const TriangleEdgeModel &,
                                    const EdgeModel &, EdgeVectors_t &,
                                    EdgeVectors_t &) const;
  void GetTriangleElementFieldPairs(const Triangle &, const TriangleEdgeModel &,
                                    const EdgeModel &, const EdgeModel &,
...
private:
  ...
  typedef std::tuple<DerivativeVectorPairs_t, DerivativeWeightPairs_t>
      DerivativeTuplePairs_t;

  typedef std::array<Vector<DoubleType>, 3> EdgePairVectors_t;
  typedef std::array<EdgePairVectors_t, 3> DerivativeEdgePairVectors_t;

  void GetFieldPairs(const Triangle &, const TriangleEdgeModel &,
                     const std::vector<DoubleType> &, TuplePairs_t &) const;
  void GetTriangleElementField(const Triangle &, const TriangleEdgeModel &,
                               const std::vector<DoubleType> &,
                               EdgeVectors_t &) const;

  const EdgePairVectors_t &
  GetEdgePairVectors(const Triangle &, const std::vector<DoubleType> &) const;
  const DerivativeEdgePairVectors_t &
  GetDerivativeEdgePairVectors(const Triangle &,
                               const std::vector<DoubleType> &,
                               const std::vector<DoubleType> &) const;

  void GetDerivativeFieldPairs(const Triangle &, const TriangleEdgeModel &,
                               const EdgeModel &, const EdgeModel &,
                               DerivativeTuplePairs_t &) const;

  void PopulateEdgeData(const Triangle &, const EdgeModel &,
  ...
  typedef std::vector<TriangleElementFieldMatrixHolder<DoubleType>>
      dense_matrix_vec_t;
