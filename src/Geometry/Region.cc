...
template <typename DoubleType>
const TriangleElementField<DoubleType> &
Region::GetTriangleElementField() const {
...
template <typename DoubleType>
const TetrahedronElementField<DoubleType> &
Region::GetTetrahedronElementField() const {
...
template <> Region::GeometryField<double> &Region::GetGeometryField() const {
  return GeometryField_double;
}

#ifdef DEVSIM_EXTENDED_PRECISION
template <> Region::GeometryField<float128> &Region::GetGeometryField() const {
...
Region::GetTriangleEdgeModel(const std::string &nm) const {
...
Region::GetTetrahedronEdgeModel(const std::string &nm) const {
...
Region::AddTetrahedronEdgeModel(TetrahedronEdgeModel *emp) {
...
void Region::SignalCallbacks(const std::string &str) {
  ...
  for (; lit != lend; ++lit) {
    dsAssert(!(nodeModels.count(*lit) && edgeModels.count(*lit) &&
               triangleEdgeModels.count(*lit) &&
               tetrahedronEdgeModels.count(*lit)),
             "UNEXPECTED");
...
size_t Region::GetEdgeIndexOnTriangle(const Triangle &t,
...
void Region::SetMaterial(const std::string &new_material) {
  ...
  const MaterialDB::ParameterDataMap_t &old_data =
      md.GetOpenEntries(materialName);

  //// We can no longer rely on anything based on our old material name
  for (MaterialDB::ParameterDataMap_t::const_iterator it = old_data.begin();
       it != old_data.end(); ++it) {
  ...
  const MaterialDB::ParameterDataMap_t &global_data =
      md.GetOpenEntries("global");

  //// Check for global data.  If old material did not define it, then signal
  ///callback / The next model evaluation will detect if it is available from
  ///the db
  for (MaterialDB::ParameterDataMap_t::const_iterator it = global_data.begin();
       it != global_data.end(); ++it) {
    const std::string &parameter_name = it->first;
    if (!(gd.GetDBEntryOnRegion(this, parameter_name).first)) {
  ...
  for (Device::InterfaceList_t::const_iterator it = iml.begin();
       it != iml.end(); ++it) {
    Interface *interface = it->second;
    if ((interface->GetRegion0() == this) ||
        (interface->GetRegion1() == this)) {
      const Interface::NameToInterfaceNodeModelMap_t &iml =
          interface->GetInterfaceNodeModelList();
      for (Interface::NameToInterfaceNodeModelMap_t::const_iterator it =
               iml.begin();
           it != iml.end(); ++it) {
        it->second->MarkOld();
      }
...
#ifdef DEVSIM_EXTENDED_PRECISION
template <> ModelExprDataCachePtr<float128> Region::GetModelExprDataCache() {
