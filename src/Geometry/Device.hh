...
public:
  ...
  typedef std::map<size_t, std::vector<InterfacePtr>>
      CoordinateIndexToInterface_t;
  typedef std::map<size_t, std::vector<ContactPtr>> CoordinateIndexToContact_t;

  Device(std::string, size_t);

  /// If we ever add a delete method.  Interfaces would need to be removed
  /// automatically when their underlying regions are removed.
  ...
  template <typename DoubleType>
  void
  ACUpdate(const std::vector<dsMath::ComplexDouble_t<DoubleType>> & /*result*/);
  template <typename DoubleType>
  void NoiseUpdate(
      const std::string & /*output*/,
      const std::vector<PermutationEntry> & /*permvec*/,
      const std::vector<dsMath::ComplexDouble_t<DoubleType>> & /*result*/);
  ...
  template <typename DoubleType>
  void ContactAssemble(dsMath::RealRowColValueVec<DoubleType> &,
                       dsMath::RHSEntryVec<DoubleType> &, PermutationMap &,
                       dsMathEnum::WhatToLoad, dsMathEnum::TimeMode);

  template <typename DoubleType>
  void InterfaceAssemble(dsMath::RealRowColValueVec<DoubleType> &,
                         dsMath::RHSEntryVec<DoubleType> &, PermutationMap &,
                         dsMathEnum::WhatToLoad, dsMathEnum::TimeMode);

  template <typename DoubleType>
  void RegionAssemble(dsMath::RealRowColValueVec<DoubleType> &,
  ...
  const CoordinateList_t &GetCoordinateList() const { return coordinateList; }

  const RegionList_t &GetRegionList() const { return regionList; }

  const InterfaceList_t &GetInterfaceList() const { return interfaceList; }

  const ContactList_t &GetContactList() const { return contactList; }
  ...
  void SignalCallbacksOnInterface(const std::string & /*str*/,
                                  const Region *) const;
